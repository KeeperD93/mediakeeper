...
| Version | Supportée |
|---|---|
| `0.9.x` (actuelle) | ✅ Dernière mineure uniquement |
| `< 0.9` | ❌ Merci de mettre à jour avant tout signalement |

Jusqu'à la sortie de `1.0.0`, **seule la dernière mineure `0.9.x` reçoit les correctifs de sécurité**. Après `1.0.0`, la même politique s'applique : seule la dernière ligne stable est supportée.
