    ...
    <div class="ob-folders-list">
      <div v-for="(folder, i) in folders" :key="folder._id" class="ob-folder-card">
        <div class="ob-folder-card-top">
          <input v-model="folder.label" class="ob-input ob-folder-name-input" :placeholder="$t('onboarding.folderNamePlaceholder')" />
          <button class="ob-folder-remove" :title="$t('onboarding.folderRemove')" @click="$emit('remove-folder', i)">
            <X :size="13" />
          </button>
        </div>
        <div class="ob-folder-path-row">
          <input v-model="folder.path" class="ob-input ob-mono ob-folder-path-input" readonly :placeholder="$t('onboarding.folderSelectPath')" @click="$emit('open-browser', folder._id)" />
          <button class="ob-folder-browse-btn" :title="$t('onboarding.folderSelectPath')" @click="$emit('open-browser', folder._id)">
        ...
        <div v-if="browseOpen === folder._id" class="ob-browse-panel">
          <div class="ob-browse-bc">
            ...
            <template v-for="(seg, si) in crumbs" :key="si">
              ...
              <span class="ob-browse-crumb" @click="$emit('browse-to', '/' + crumbs.slice(0, si + 1).join('/'))">{{ seg }}</span>
            </template>
          </div>
          <div class="ob-browse-list">
            <div v-if="browseLoading" class="ob-browse-empty"><span class="ob-spin" /></div>
            <div v-else-if="!browseDirs.length" class="ob-browse-empty">{{ $t('onboarding.noSubfolders') }}</div>
            <div v-for="d in browseDirs" :key="d.path" class="ob-browse-item" @click="$emit('browse-to', d.path)">
              <Folder :size="13" :stroke-width="1.8" />
              <span class="ob-browse-name">{{ d.name }}</span>
              <button class="ob-browse-select" @click.stop="$emit('select-browse-path', i, d.path)">{{ $t('onboarding.folderSelect') }}</button>
            </div>
          </div>
          <div v-if="browsePath !== '/'" class="ob-browse-current">
            <button class="ob-browse-use-current" @click="$emit('select-browse-path', i, browsePath)">
...
const crumbs = computed(() =>
  props.browsePath.replace(/^\/+/, '').split('/').filter(Boolean)
)
</script>

<style scoped>
.ob-step-icon-folders { background: rgba(245,158,11,.1); }
.ob-step-icon-folder-ic { color: #f59e0b; }
</style>
