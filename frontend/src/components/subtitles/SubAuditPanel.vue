        ...
        <div class="sub-audit-modal">
          ...
          <div v-if="!results && !running" class="sub-audit-config">
            <div class="sub-audit-field">
              ...
              <div class="sub-audit-checks">
                <label>
                  <input v-model="langs" type="checkbox" value="fre" />
                  FR
                </label>
                <label>
                  <input v-model="langs" type="checkbox" value="eng" />
                  EN
                </label>
                <label>
                  <input v-model="langs" type="checkbox" value="spa" />
                  ES
                </label>
                <label>
                  <input v-model="langs" type="checkbox" value="ger" />
                  DE
                </label>
              </div>
            </div>
            <div class="sub-audit-field">
              ...
              <div class="sub-audit-checks">
                <label>
                  <input v-model="checks" type="checkbox" value="missing" />
                  {{ $t('subtitles.filterMissing') }}
                </label>
                <label>
                  <input v-model="checks" type="checkbox" value="forced" />
                  {{ $t('subtitles.missingForced') }}
                </label>
                <label>
                  <input v-model="checks" type="checkbox" value="image_only" />
                  {{ $t('subtitles.imageOnlySubs') }}
                </label>
                <label>
                  <input v-model="checks" type="checkbox" value="encoding" />
                  {{ $t('subtitles.encodingIssues') }}
                </label>
              </div>
            </div>
            <button
              class="sub-audit-start"
              :disabled="!langs.length || !checks.length"
              @click="startAudit"
            >
          ...
          <div v-if="results" class="sub-audit-results">
            ...
            <div class="sub-audit-summary">
              <div
                v-for="(count, lang) in results.summary?.missing || {}"
                :key="'miss-' + lang"
                class="sub-audit-sum-card"
              >
                <span class="sub-audit-sum-val">{{ count }}</span>
                <span class="sub-audit-sum-label">
                  {{ $t('subtitles.filterMissing') }} {{ lang.toUpperCase() }}
                </span>
              </div>
            ...
            <div class="sub-audit-list">
              <div
                v-for="item in (results.items || []).slice(0, 100)"
                :key="item.item_id"
                class="sub-audit-item"
              >
                <div class="sub-audit-item-info">
                  <span class="sub-audit-item-name">
            ...
            <!-- Actions -->
            <button class="sub-audit-restart" @click="results = null">
              {{ $t('subtitles.startAudit') }}
            </button>
          </div>
  ...
  try {
    ...
    _timer = setInterval(async () => {
      try {
        ...
        /* silent: audit progress poll, retries next tick */
      }
    }, 2000)
  } catch {
    running.value = false
