    ...
    <transition name="sub-fade">
      <div v-if="visible" class="so-overlay" @click.self="$emit('close')">
        <div class="so-modal so-solid">
          <!-- Header -->
          <div class="so-header">
            <img :src="'/api/emby/image/' + seriesPoster" class="so-poster" alt="" @error="$event.target.style.display='none'" />
          ...
          <div class="so-episodes">
            <template v-for="s in seasons" :key="s.num">
              ...
              <div class="so-season-header" @click="toggleSeason(s.num)">
                <ChevronDown :class="{ 'so-chev-open': openSeasons[s.num] }" class="so-season-chev" :size="14" :stroke-width="2.5" />
                ...
                <div class="so-ep-pills">
                  <span v-for="(has, lang) in s.globalTags" :key="lang" class="sg-pill" :class="[has ? 'sg-pill-ok' : 'sg-pill-miss']" :style="pillStyle(lang, has)">
              ...
              <template v-if="openSeasons[s.num]">
                <template v-for="ep in s.episodes" :key="ep.item_id">
                  <div class="so-ep-row so-ep-indent" :class="{ 'so-ep-active': selectedEp && selectedEp.item_id === ep.item_id }" @click="selectEpisode(ep)">
                    <span class="so-ep-tag">S{{ ep.season || 0 }}E{{ ep.episode || 0 }}</span>
                    <span class="so-ep-name">{{ ep.name }}</span>
                    <div class="so-ep-pills">
                      <span v-for="(has, lang) in ep.subtitle_status" :key="lang" class="sg-pill" :class="[has ? 'sg-pill-ok' : 'sg-pill-miss']" :style="pillStyle(lang, has)">
                        <span v-if="has" class="sg-pill-dot" :style="dotStyle(lang)" />
                        {{ langShort(lang) }}
                      </span>
                    </div>
                    <span v-if="selectedEp && selectedEp.item_id === ep.item_id && epSubs === null" class="mk-spin so-ep-loading" />
                    <ChevronRight v-else class="so-ep-chevron" :class="{ 'so-ep-chevron-open': selectedEp && selectedEp.item_id === ep.item_id && epSubs !== null }" :size="14" />
                  </div>
                  ...
                  <div class="so-detail-slot" :class="{ 'so-detail-slot--open': selectedEp && selectedEp.item_id === ep.item_id && epSubs !== null }">
                    <div class="so-detail-clip">
                    <div v-if="selectedEp && selectedEp.item_id === ep.item_id" class="so-detail so-solid">
                      <div class="so-detail-header">
                        <span class="so-detail-tag">S{{ selectedEp.season }}E{{ selectedEp.episode }}</span>
                      ...
                      <div class="sd-section">
                        ...
                        <div v-if="epSubs === null" class="sd-loading"><span class="mk-spin mk-spin-14" /></div>
                        <div v-else-if="!epSubs.length" class="sd-empty">{{ $t('subtitles.noSubs') }}</div>
                        <div v-else class="sd-sub-list">
                          <div v-for="s2 in epSubs" :key="s2.index" class="sd-sub-row">
                            <input v-if="!s2.is_external" type="checkbox" class="sd-sub-check" :checked="removeSelection.includes(s2.index)" :disabled="removing" @change="toggleRemove(s2.index)" />
                            ...
                            <span class="sd-sub-badge" :class="s2.is_external ? 'badge-ext' : 'badge-emb'">{{ s2.is_external ? $t('subtitles.external') : $t('subtitles.embedded') }}</span>
                            <span v-if="s2.is_forced" class="sd-sub-badge badge-forced">{{ $t('subtitles.forced') }}</span>
                            <span v-if="s2.is_image_based" class="sd-sub-badge badge-pgs">PGS</span>
                            <button v-if="s2.is_external && s2.path" class="sd-sub-del" :disabled="removing" :title="$t('common.delete')" @click="deleteSub(s2)">
                              <Trash2 :size="12" />
                            </button>
                          </div>
                        </div>
                      ...
                      <div class="sd-section">
                        ...
                        <div class="so-detail-slot" :class="{ 'so-detail-slot--open': !epSearching && epResults.length > 0 }">
                          <div class="so-detail-clip">
                            <div class="so-results-inner">
                              ...
                              <div class="sd-os-results">
                                <SubOsResultRow
                                  v-for="r in epResults" :key="r.file_id"
                                  :result="r"
                                  :downloading="downloading === r.file_id"
                                  :selectable="epResults.length >= 2"
                                  :selected="compareSelection.some(s3 => s3.file_id === r.file_id)"
...
function toggleSeason(num) { openSeasons[num] = !openSeasons[num] }

const {
  selectedEp, epSubs, epAudio, epResults, epSearching,
...
watch(() => props.visible, (v) => { if (!v) resetEpisode() })
