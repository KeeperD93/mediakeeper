<template>
  <div class="sub-top-right">
    ...
    <div v-if="profiles.length" class="sub-profile-select">
      ...
      <button
        class="sub-profile-edit-btn"
        :title="$t('subtitles.profiles')"
        @click="$emit('edit-profiles')"
      >
    ...
    <span
      v-if="defaultProfile && defaultProfile.auto_download"
      class="sub-auto-badge"
      :title="$t('subtitles.autoDownloadEnabled')"
    >
      <Zap :size="12" />
      AUTO
    </span>
    ...
    <div class="sub-quota" :title="$t('subtitles.downloadsLeft')">
      <template v-if="quota && !quota.error">
        <div class="sub-quota-bar">
          <div class="sub-quota-fill" :style="{ width: quotaPct + '%', background: quotaColor }" />
        </div>
        <span class="sub-quota-text">
          {{ quota.remaining_downloads }}/{{ quota.allowed_downloads }}
        </span>
      </template>
...
defineEmits(['edit-profiles', 'audit'])
...
.sub-auto-badge {
  ...
  border: 0.5px solid rgb(var(--color-success-rgb), 0.15);
}
...
.sub-banner-warn {
  ...
  border-color: rgb(var(--color-warning-rgb), 0.15);
}
.glass-card {
