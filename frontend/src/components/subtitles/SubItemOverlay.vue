        ...
        <div class="si-modal">
          ...
          <div class="si-header">
            ...
            <div class="si-header-info">
              ...
              <span class="si-meta">
                <span v-if="item.type === 'Episode'" class="si-tag">
            ...
            <div v-else class="sd-sub-list">
              <div v-for="s in subs" :key="s.index" class="sd-sub-row">
                <input
                  ...
                  type="checkbox"
                  class="sd-sub-check"
                  :checked="removeSelection.includes(s.index)"
                  :disabled="removing"
                  @change="toggleRemove(s.index)"
                />
          ...
          <div class="sd-section">
            ...
            <!-- Correction decalage -->
            <div
              v-if="lastDlResult && lastDlResult.desync && lastDlResult.desync.desynced"
              class="si-shift-bar"
            >
              ...
              <button
                class="si-shift-auto"
                :title="$t('subtitles.shiftAuto')"
                @click="shiftMs = Math.round(-lastDlResult.desync.delta_sec * 1000)"
              >
                Auto ({{ lastDlResult.desync.delta_sec > 0 ? '-' : '+'
                }}{{ Math.abs(Math.round(lastDlResult.desync.delta_sec * 1000)) }}ms)
              </button>
            </div>
          </div>
        </div>
