<template>
  <div class="sub-panel sub-panel--search">
    ...
    <div class="ss-mode-toggle">
      <button class="ss-mode-btn" :class="{ active: searchMode === 'subtitles' }" @click="searchMode = 'subtitles'">
        <Search :size="14" />
        {{ $t('subtitles.searchSubs') }}
      </button>
      <button class="ss-mode-btn" :class="{ active: searchMode === 'streams' }" @click="searchMode = 'streams'">
        <Calendar :size="14" />
    ...
    <template v-if="searchMode === 'subtitles'">
      <div class="ss-card sub-search-card">
        <div class="sub-search-row">
          <input v-model="searchQuery" class="sub-search-input" :placeholder="$t('subtitles.searchPlaceholder')" @keydown.enter="doTmdbSearch" />
          ...
          <select v-model="searchLangs" class="sub-filter-sel">
            <option :value="defaultLanguagesParam">{{ defaultLanguages.map(l => langShort(l)).join(' + ') }}</option>
            <option v-for="lang in defaultLanguages" :key="lang" :value="langShort(lang).toLowerCase()">{{ langShort(lang) }}</option>
            <option value="">{{ $t('subtitles.typeAll') }}</option>
          </select>
          <button class="sub-search-go" :disabled="searching || !searchQuery.trim()" @click="doTmdbSearch">
            <Search v-if="!searching" :size="16" />
            <span v-else class="mk-spin mk-spin-16" />
          </button>
        </div>
        <div v-if="!tmdbResults.length && !searchResults.length && !searching" class="sub-search-hint">{{ $t('subtitles.searchHint') }}</div>
      </div>
      ...
      <div v-if="selectedTmdb" class="ss-card sub-selected-banner">
        <img v-if="selectedTmdb.poster" :src="selectedTmdb.poster" class="sub-selected-poster" alt="" />
      ...
      <div v-if="searchResults.length" class="ss-card sub-os-card">
        <div class="sub-section-title">{{ searchResults.length }} {{ $t('subtitles.results') }}</div>
        <div class="sub-os-results">
          <SubOsResultRow v-for="r in searchResults" :key="r.file_id" :result="r" :downloading="downloading === r.file_id" @download="downloadSearchResult($event)" />
        </div>
      </div>

      <SubDownloadModal :visible="!!downloadModal" :default-path="downloadDest" @close="downloadModal = null" @confirm="confirmDownload" />
    </template>
    ...
    <template v-else>
      <div class="ss-card sub-search-card">
        <div class="sub-search-row">
          <input v-model="streamQuery" class="sub-search-input" :placeholder="$t('subtitles.streamSearchPlaceholder')" @keydown.enter="doStreamSearch" />
          ...
          <button class="sub-search-go" :disabled="streamSearching || !streamQuery.trim()" @click="doStreamSearch">
      ...
      <div v-if="streamResults.length" class="ss-stream-results">
        ...
        <div class="ss-batch-bar">
          ...
          <button v-if="selectedStreams.length" class="ss-batch-del" :disabled="batchRemoving" @click="batchRemove">
            <Trash2 :size="13" />
        ...
        <div v-for="item in streamResults" :key="item.item_id" class="ss-stream-item ss-card">
          <div class="ss-stream-header">
            <img :src="'/api/emby/image/' + (item.poster_id || item.item_id)" class="ss-stream-poster" alt="" loading="lazy" @error="$event.target.style.display='none'" />
            <div class="ss-stream-info">
              <span class="ss-stream-name">{{ item.series_name ? item.series_name + ' — ' : '' }}{{ item.name }}</span>
              <span class="ss-stream-meta">
                <span class="ss-stream-tag" :class="item.type === 'Movie' ? 'tag-movie' : 'tag-ep'">{{ item.type === 'Movie' ? $t('subtitles.typeMovie') : `S${item.season||0}E${item.episode||0}` }}</span>
          ...
          <div class="ss-stream-list">
            <div v-for="s in item.matching_streams" :key="s.index" class="ss-stream-row">
              <input type="checkbox" class="ss-stream-check"
                :checked="isStreamSelected(item.item_id, s.index)"
                @change="toggleStreamSelect(item.item_id, s.index, item, s)" />
              <span class="ss-stream-type-badge" :class="s.type === STREAM_TYPE.AUDIO ? 'badge-audio' : 'badge-sub'">{{ s.type === STREAM_TYPE.AUDIO ? 'AUD' : 'SUB' }}</span>
              ...
              <span v-if="s.channels" class="ss-stream-channels">{{ formatChannels(s.channels) }}</span>
              ...
              <button class="ss-del-btn" :title="$t('subtitles.removeStream')" @click="removeSingleStream(item.item_id, s.index, s)">
                <Trash2 :size="12" />
              </button>
            </div>
          </div>
        ...
        <div v-if="streamTotal > streamResults.length" class="ss-more">
          <button class="ss-more-btn" :disabled="streamSearching" @click="loadMoreStreams">{{ $t('subtitles.loadMore') }}</button>
        </div>
      </div>
...
const LANG_SHORT = { fre: 'FR', eng: 'EN', spa: 'ES', ger: 'DE', ita: 'IT', por: 'PT', jpn: 'JP', chi: 'ZH', kor: 'KO', rus: 'RU', ara: 'AR', hin: 'HI', nld: 'NL', dut: 'NL', pol: 'PL', tur: 'TR', swe: 'SV', dan: 'DA', nor: 'NO', fin: 'FI', ces: 'CZ', cze: 'CZ', ron: 'RO', rum: 'RO', hun: 'HU', ell: 'EL', gre: 'EL', heb: 'HE', tha: 'TH', vie: 'VI', ind: 'ID', ukr: 'UK' }
...
async function doTmdbSearch() {
  ...
  tmdbResults.value = []; searchResults.value = []; selectedTmdb.value = null
  try {
    const endpoint = searchType.value === 'tv' ? '/api/media/tmdb/search/tv' : '/api/media/tmdb/search/movie'
  ...
  } catch { await doDirectSearch(searchQuery.value.trim()) }
  searching.value = false
}

async function selectTmdb(media) {
  selectedTmdb.value = media; tmdbResults.value = []; searching.value = true; searchResults.value = []
  try {
    const d = await apiPost('/api/subtitles/search', { query: media.title, tmdb_id: String(media.id), languages: searchLangs.value })
...
async function doDirectSearch(query) {
  try {
    ...
    if (d && d.results) searchResults.value = d.results
    if (!searchResults.value.length) showToast(t('common.noResults'), TOAST_TYPE.INFO)
  } catch (e) { console.error('[SubSearchTab.doDirectSearch] search failed', e); showToast(t('common.error'), TOAST_TYPE.ERR) }
}

function clearTmdbSelection() { selectedTmdb.value = null; searchResults.value = []; tmdbResults.value = [] }
...
async function confirmDownload(dest) {
  ...
  const r = downloadModal.value; downloading.value = r.file_id; downloadModal.value = null
  try {
    const d = await apiPost('/api/subtitles/download', {
      file_id: r.file_id, destination: dest, media_name: r.feature_title || '',
      language: r.language || '', subtitle_id: r.subtitle_id || '', file_name: r.file_name || '',
      quality_score: r.quality_score || 0, hash_match: r.hash_match || false,
      hearing_impaired: r.hearing_impaired || false, foreign_parts_only: r.foreign_parts_only || false,
      from_trusted: r.from_trusted || false, ai_translated: r.ai_translated || false,
    })
    if (d && d.success) { showToast(t('subtitles.downloaded'), TOAST_TYPE.OK); loadQuota() }
...
function formatChannels(ch) {
  if (ch === 1) return 'Mono'; if (ch === 2) return 'Stereo'; if (ch === 6) return '5.1'; if (ch === 8) return '7.1'; return `${ch}ch`
}
