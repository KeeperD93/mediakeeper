<template>
  <div class="sub-panel">
    ...
    <div class="sub-hist-filters glass-card">
      <input v-model="searchText" class="sub-filter-input" :placeholder="$t('subtitles.searchLibrary')" @input="debounceLoad" />
    ...
    <!-- Loading -->
    <div v-if="loading && !history.length" class="sub-center"><span class="mk-spin mk-spin-24" /></div>
      ...
      <div v-for="dl in filteredHistory" :key="dl.id" class="sub-hist-row glass-card">
        <div class="sub-hist-main">
          ...
          <span class="sub-hist-meta">
            <span class="sub-hist-lang" :class="'lang-' + dl.language">{{ dl.language.toUpperCase() }}</span>
            <SubQualityStars v-if="dl.quality_score" :score="dl.quality_score" />
            <span v-if="dl.from_trusted" class="sub-tag tag-trusted">{{ $t('subtitles.trusted') }}</span>
...
const filteredHistory = computed(() => {
  ...
  return history.value.filter(d =>
    d.media_name.toLowerCase().includes(q) || (d.file_name || '').toLowerCase().includes(q)
  )
})
...
<style scoped>
.sub-hist-filters { display: flex; gap: 8px; padding: 10px 14px; margin-bottom: 12px; flex-wrap: wrap; }
.sub-filter-input {
  flex: 1; min-width: 160px; padding: 8px 12px; border-radius: var(--radius-btn); font-size: var(--text-sm);
  background: var(--surface-2); border: .5px solid var(--border-strong);
  color: var(--text-primary); outline: none; font-family: inherit;
}
.sub-filter-input:focus { border-color: rgba(var(--accent-rgb),.4); }
.sub-filter-sel {
  padding: 8px 10px; border-radius: var(--radius-btn); font-size: var(--text-2xs);
  background: var(--surface-2); border: .5px solid var(--border-strong);
  color: var(--text-primary); outline: none; font-family: inherit; cursor: pointer;
}
.sub-filter-sel option { background: var(--bg-secondary); color: var(--text-primary); }

.sub-center { display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 60px; gap: 12px; }

.sub-hist-list { display: flex; flex-direction: column; gap: 4px; }
.sub-hist-row { display: flex; align-items: center; justify-content: space-between; gap: 16px; padding: 10px 14px; }
.sub-hist-main { flex: 1; min-width: 0; }
.sub-hist-name { display: block; font-size: var(--text-sm); font-weight: var(--font-regular); color: var(--text-primary); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.sub-hist-meta { display: flex; align-items: center; gap: 5px; flex-wrap: wrap; margin-top: 3px; }
.sub-hist-lang { font-size: var(--text-3xs); font-weight: var(--font-bold); padding: 2px 6px; border-radius: 4px; }
...
.sub-hist-right { display: flex; flex-direction: column; align-items: flex-end; gap: 2px; flex-shrink: 0; }
.sub-hist-file { font-size: var(--text-3xs); color: var(--text-muted); max-width: 200px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.sub-hist-date { font-size: .58rem; color: var(--text-muted); }
.sub-hist-source { font-size: .52rem; color: var(--text-muted); text-transform: uppercase; letter-spacing: .3px; }

.sub-hist-more { display: flex; justify-content: center; padding: 16px; }
.sub-hist-more-btn {
  padding: 8px 24px; border-radius: var(--radius-btn); font-size: var(--text-xs);
  background: var(--surface-2); border: .5px solid var(--border-strong);
  color: var(--text-secondary); cursor: pointer; font-family: inherit;
}
.sub-hist-more-btn:hover { background: rgba(255,255,255,.08); }
.sub-hist-more-btn:disabled { opacity: .4; }
