<template>
  ...
  <!-- ═══ GRID MODE ═══ -->
  <div v-if="mode === 'grid'" class="sg-card" :class="{ 'sg-selected': expanded, 'sg-hovered': isHovered }"
    @mouseenter="isHovered = true" @mouseleave="isHovered = false" @click="$emit('toggle')">

    <!-- Poster -->
    <div class="sg-poster-wrap">
      <img :src="'/api/emby/image/' + (item.poster_id || item.item_id)" class="sg-poster" alt="" loading="lazy" @error="$event.target.style.display='none'" />
      <div class="sg-gradient" />

      <!-- Tag badge -->
      <span class="sg-tag" :class="item.type === 'Movie' ? 'sg-tag-movie' : 'sg-tag-ep'">
        {{ item.type === 'Movie' ? $t('subtitles.typeMovie') : item._isGroup ? `${item._episodeCount} ep` : `S${item.season||0}E${item.episode||0}` }}
      </span>
      ...
      <input v-if="selectable" type="checkbox" class="sg-check" :checked="isSelected" @click.stop @change.stop="$emit('select-item')" />
      ...
      <div class="sg-bottom">
        ...
        <div class="sg-pills">
          <span v-for="(has, lang) in item.subtitle_status" :key="lang" class="sg-pill" :class="[has ? 'sg-pill-ok' : 'sg-pill-miss']" :style="pillStyle(lang, has)">
    ...
    <div v-if="expanded && !item._isGroup" class="sg-actions">
      ...
      <button v-if="item.type === 'Episode' && item.series_name" class="sg-action-btn sg-action-matrix" @click.stop="$emit('show-matrix', item)">
  ...
  <!-- ═══ LIST MODE ═══ -->
  <div v-else class="sl-row" :class="{ 'sl-hovered': isHovered, 'sl-expanded': expanded }"
    @mouseenter="isHovered = true" @mouseleave="isHovered = false" @click="$emit('toggle')">

    <input v-if="selectable" type="checkbox" class="sl-check" :checked="isSelected" @click.stop @change.stop="$emit('select-item')" />
    <img :src="'/api/emby/image/' + (item.poster_id || item.item_id)" class="sl-poster" alt="" loading="lazy" @error="$event.target.style.display='none'" />
    <div class="sl-info">
      <div class="sl-name">{{ item._isGroup ? item.name : (item.series_name ? item.series_name + ' — ' : '') + item.name }}</div>
  ...
  <transition name="sub-slide">
    <div v-if="expanded && !item._isGroup && (subs !== null || results.length)" class="sd-panel sd-solid">
      <!-- Existing subtitles -->
      <div class="sd-section">
        ...
        <div v-else class="sd-sub-list">
          <div v-for="s in subs" :key="s.index" class="sd-sub-row">
            ...
            <button v-if="s.is_external && s.path" class="sd-sub-del" :title="$t('common.delete')" @click.stop="$emit('delete-sub', s)">
              <Trash2 :size="12" />
            </button>
            <button v-if="!s.is_external" class="sd-sub-del" :title="$t('subtitles.removeStream')" @click.stop="$emit('remove-stream', { item_id: item.item_id, stream_index: s.index, type: STREAM_TYPE.SUBTITLE, language: s.language })">
              <Trash2 :size="12" />
            </button>
          </div>
        </div>
      ...
      <div v-if="audioStreams && audioStreams.length" class="sd-section">
        ...
        <div class="sd-sub-list">
          <div v-for="a in audioStreams" :key="a.index" class="sd-sub-row">
            ...
            <button class="sd-sub-del" :title="$t('subtitles.removeStream')" @click.stop="$emit('remove-stream', { item_id: item.item_id, stream_index: a.index, type: STREAM_TYPE.AUDIO, language: a.language })">
              <Trash2 :size="12" />
            </button>
          </div>
        </div>
      ...
      <div v-if="results.length || searching" class="sd-section">
        <div v-if="results.length >= 2" class="sub-cmp-hint">
          ...
          <button v-if="compareSelection.length === 2" class="sub-cmp-go" @click.stop="$emit('compare', compareSelection)">
            {{ $t('subtitles.compare') }}
          </button>
        </div>
        ...
        <SubDesyncBanner v-if="lastDownloadResult" :desync="lastDownloadResult.desync" :encoding="lastDownloadResult.encoding" />
      </div>
    </div>
  </transition>
  </div>
...
defineEmits(['toggle', 'search', 'download', 'delete-sub', 'remove-stream', 'show-matrix', 'compare', 'select-item'])

const isHovered = ref(false)
const compareSelection = ref([])

const hasMissing = computed(() =>
  !Object.values(props.item.subtitle_status || {}).every(v => v)
)

function langShort(code) {
  return { fre: 'FR', eng: 'EN', spa: 'ES', ger: 'DE', ita: 'IT', por: 'PT', jpn: 'JP', chi: 'ZH', kor: 'KO', rus: 'RU', ara: 'AR', hin: 'HI', nld: 'NL', dut: 'NL', pol: 'PL', tur: 'TR', swe: 'SV', dan: 'DA', nor: 'NO', fin: 'FI', ces: 'CZ', cze: 'CZ', ron: 'RO', rum: 'RO', hun: 'HU', ell: 'EL', gre: 'EL', heb: 'HE', tha: 'TH', vie: 'VI', ind: 'ID', ukr: 'UK' }[code] || code.slice(0, 2).toUpperCase()
}
...
function langColor(code) {
  ...
  _langColorCache[code] = { hue, color: `hsl(${hue}, 65%, 70%)`, bg: `hsla(${hue}, 65%, 55%, 0.15)` }
