    ...
    <div class="pt-settings-avatar-block">
      ...
      <div class="pt-settings-avatar-actions">
        ...
        <div class="pt-settings-avatar-buttons">
          <button type="button" class="pt-settings-btn" :disabled="uploading" @click="triggerFile">
            <Upload :size="14" />
            {{
              profileData?.avatar_is_custom
...
const avatarFxClass = computed(() =>
  avatarFxKey.value ? `gc-avatar-fx--${avatarFxKey.value}` : '',
)
...
const unameFlag = computed(() => {
  ...
  if (c.pending) {
    return {
      show: true,
      cls: 'pt-settings-uname-flag--warn',
  ...
  if (c.reason === 'taken') {
    return {
      show: true,
      cls: 'pt-settings-uname-flag--err',
...
async function onFile(e) {
  ...
  try {
    ...
    if (detail === 'avatar_too_large')
      showToast(t('portal.settings.identity.avatarTooLarge'), TOAST_TYPE.ERR)
    else if (detail?.startsWith('avatar_'))
      showToast(t('portal.settings.identity.avatarBadFormat'), TOAST_TYPE.ERR)
