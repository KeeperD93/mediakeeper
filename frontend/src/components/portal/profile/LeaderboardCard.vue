<template>
  <div v-if="entries.length || widget" class="gc-lb-box" :class="{ 'gc-lb-box--widget': widget }">
    <h4 class="gc-box-title" :class="{ 'gc-lb-widget-title': widget }">{{ $t('portal.profile.ranking') }}</h4>
    <div class="gc-lb-rows" :data-full="entries.length >= 16 || null">
      <component
        :is="entry.user_id ? 'router-link' : 'div'"
        v-for="entry in entries"
        :key="entry.rank"
        :to="entry.user_id ? { name: 'portal-user-profile', params: { id: entry.user_id } } : undefined"
        class="gc-lb-row"
        :class="{
      ...
      >
        ...
        <div class="gc-lb-info">
          ...
          <span class="gc-lb-tier">
            ...
            <em v-if="entry.selected_title" class="gc-lb-custom-title" :class="entry.title_tier ? `gc-lb-custom-title--rarity-${tierToRarity(entry.title_tier)}` : ''">· {{ $t(entry.selected_title) }}</em>
          </span>
        </div>
        <span
          class="gc-lb-move"
        ...
        >
          <ChevronUp
            v-if="entry.movement > 0"
            :size="9"
            :stroke-width="3.5"
          />
          <ChevronDown
            v-else-if="entry.movement < 0"
            :size="9"
            :stroke-width="3.5"
          />
          <span v-else>—</span>
...
const RARITY_BY_TIER = { 1: 'common', 2: 'uncommon', 3: 'rare', 4: 'epic', 5: 'legendary', 6: 'mythic' }
...
<style>
@import '@/assets/styles/portal/leaderboard-card.css';
</style>
