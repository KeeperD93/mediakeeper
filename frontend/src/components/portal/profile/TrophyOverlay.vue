          ...
          <div class="ta-header-left">
            ...
            <div class="ta-progress-wrap">
              ...
              <div class="ta-progress-text">
                <strong>{{ unlockedCount }}</strong> / {{ totalCount }}
        ...
        <div class="ta-body">
          ...
          <div class="ta-sidebar">
            <div class="ta-search">
              ...
              />
              <button v-if="searchQuery" class="ta-search-clear" @click="searchQuery = ''">×</button>
            </div>
            <button class="ta-reward-chip"
            ...
            <div v-for="cat in categoriesWithStats" :key="cat.id"
              class="ta-cat"
            ...
            >
              ...
              <div class="ta-cat-bar">
                <div class="ta-cat-bar-fill" :style="{ width: cat.total > 0 ? Math.round(100 * cat.unlocked / cat.total) + '%' : '0%' }" />
              </div>
            </div>
          </div>
          ...
          <div class="ta-main">
            <div v-for="ach in searchedTrophies" :key="ach.id"
              class="ta-row"
              role="button"
              tabindex="0"
            ...
            >
              <div class="ta-row-icon"
                :class="[
                  (ach.status === TROPHY_STATUS.LOCKED || ach.status === TROPHY_STATUS.PROGRESS) ? 'gc-trophy-icon--dim' : ''
                ]">
                <span class="ta-row-icon-inner"
                  :class="ach.status === TROPHY_STATUS.UNLOCKED
                    ? (ach.secret
              ...
              <div class="ta-row-info">
                <div class="ta-row-top">
                  <h4 class="ta-row-name">{{ ach.status === TROPHY_STATUS.SECRET ? '???' : $t(ach.name_key) }}</h4>
                  <span v-if="ach.status === TROPHY_STATUS.UNLOCKED" class="gc-trophy-rarity" :class="`gc-trophy-rarity--${ach.rarity}`">{{ $t(`portal.profile.rarity.${ach.rarity}`) }}</span>
                </div>
                <div v-if="ach.status !== TROPHY_STATUS.SECRET" class="ta-row-desc">{{ trophyDesc(ach) }}</div>
                <div v-else class="ta-row-desc">{{ $t('portal.profile.secretTrophy') }}</div>
                <div v-if="ach.status !== TROPHY_STATUS.SECRET && ach.threshold" class="ta-row-prog">
                  <div class="ta-row-pbar"><div class="ta-row-pfill" :class="`gc-trophy-pfill--${ach.tier_name}`" :style="{ width: Math.min(100, Math.round(100 * ach.progress / ach.threshold)) + '%' }" /></div>
                  <div class="ta-row-ptxt">{{ Math.min(ach.progress, ach.threshold) }} / {{ ach.threshold }}</div>
                </div>
              </div>
              ...
              <div class="ta-row-right">
                ...
                <div v-if="ach.title_reward"
                  ...
                  <Crown v-if="ach.category === TROPHY_CATEGORY.MASTERY && ach.status === TROPHY_STATUS.UNLOCKED" :size="14" class="ta-row-title-crown" />
                ...
                <div v-if="ach.category === TROPHY_CATEGORY.MASTERY"
                  ...
                  <span v-if="ach.status === TROPHY_STATUS.UNLOCKED" class="ta-row-title">
                    <span class="ta-row-title-name">{{ $t('portal.profile.avatarEffectReward') }}</span>
                  </span>
                </div>
                <div v-if="ach.status === TROPHY_STATUS.UNLOCKED" class="ta-row-stars">{{ '⭐'.repeat(Math.min(6, ach.stars)) }}{{ '☆'.repeat(Math.max(0, 6 - ach.stars)) }}</div>
                <div class="ta-row-xp">+{{ ach.xp_reward }} XP</div>
                <div v-if="ach.status === TROPHY_STATUS.UNLOCKED && ach.unlocked_at" class="ta-row-date">{{ new Date(ach.unlocked_at).toLocaleDateString() }}</div>
              </div>
              <button v-if="ach.status === TROPHY_STATUS.UNLOCKED"
                class="ta-row-pin"
                :class="{ 'ta-row-pin--active': pinnedIds.includes(ach.id) }"
                :disabled="!pinnedIds.includes(ach.id) && pinnedIds.length >= 5"
                :title="pinnedIds.includes(ach.id) ? $t('portal.profile.unpinBadge') : $t('portal.profile.pinBadge')"
            ...
            <div v-if="searchedTrophies.length === 0" class="ta-empty">{{ $t('portal.profile.noTrophiesInCategory') }}</div>
          </div>
        </div>
      </div>
    </div>
...
watch(
  ...
  async ([open, fid]) => {
    if (!open || !fid) { focusedId.value = null; return }
    await nextTick()
    const el = document.querySelector(`[data-trophy-id="${fid}"]`)
    if (el) {
      el.scrollIntoView({ behavior: 'smooth', block: 'center' })
      focusedId.value = fid
      setTimeout(() => { if (focusedId.value === fid) focusedId.value = null }, 2200)
    }
  },
  { immediate: true },
)
...
const searchedTrophies = computed(() => {
  ...
  return list.filter((ach) => {
    ...
    const titleReward = ach.title_reward && te(ach.title_reward) ? t(ach.title_reward).toLowerCase() : ''
...
<style>
@import '@/assets/styles/portal/trophy-overlay.css';
</style>
