<template>
  ...
  <template v-if="ach.status === TROPHY_STATUS.UNLOCKED && ach.category === TROPHY_CATEGORY.MASTERY">
    ...
    <div class="gc-secret-particles"><span v-for="i in 15" :key="'mup'+i" class="gc-uc-particle" /></div>
  </template>

  <template v-if="ach.status === TROPHY_STATUS.UNLOCKED && !ach.secret && ach.category !== TROPHY_CATEGORY.MASTERY">
    <div v-if="['platinum','mythic','supreme'].includes(ach.tier_name)" class="gc-fx-sparkles">
      <span v-for="i in (ach.tier_name === 'supreme' ? 12 : ach.tier_name === 'mythic' ? 8 : 6)" :key="i" class="gc-sparkle" />
    </div>
  ...
  <template v-if="ach.secret && ach.status === TROPHY_STATUS.UNLOCKED && ach.secret_theme">
    ...
    <template v-if="ach.secret_theme === 'first'">
      <div class="gc-secret-particles"><span v-for="i in 10" :key="'fs'+i" class="gc-first-spark" /></div>
      <div class="gc-first-burst" />
    </template>
    <!-- dawn: aurora rays + sparkles -->
    <template v-else-if="ach.secret_theme === 'dawn'">
      <div class="gc-dawn-rays" />
      <div class="gc-secret-particles"><span v-for="i in 6" :key="'dw'+i" class="gc-sparkle" /></div>
    </template>
    <!-- night: twinkling stars -->
    <div v-else-if="ach.secret_theme === 'night'" class="gc-secret-particles"><span v-for="i in 10" :key="'n'+i" class="gc-nightstar" /></div>
    <!-- xmas: snow + star + glow -->
    ...
    <div v-else-if="ach.secret_theme === 'valentine'" class="gc-secret-particles"><span v-for="i in 8" :key="'v'+i" class="gc-heart" /></div>
    ...
    <template v-else-if="ach.secret_theme === 'allnight'">
      <div class="gc-secret-particles"><span v-for="i in 8" :key="'an'+i" class="gc-nightstar" /></div>
    ...
    <div v-else-if="ach.secret_theme === 'friday13'" class="gc-secret-particles"><span v-for="i in 8" :key="'f13'+i" class="gc-blood" /></div>
    <!-- indecisive: question marks -->
    <div v-else-if="ach.secret_theme === 'indecisive'" class="gc-secret-particles"><span v-for="i in 8" :key="'ind'+i" class="gc-qmark" /></div>
    <!-- summer: sun rays + sparkles -->
    <template v-else-if="ach.secret_theme === 'summer'">
      <div class="gc-summer-rays" />
      <div class="gc-secret-particles"><span v-for="i in 6" :key="'sun'+i" class="gc-sparkle" /></div>
    </template>
    ...
    <template v-else-if="ach.secret_theme === 'pilot'">
      <div class="gc-secret-particles"><span v-for="i in 8" :key="'pil'+i" class="gc-sparkle" /></div>
    ...
    <template v-else-if="ach.secret_theme === 'king'">
      <div class="gc-secret-particles"><span v-for="i in 10" :key="'king'+i" class="gc-sparkle" /></div>
    ...
    <div v-else-if="ach.secret_theme === 'fourk'" class="gc-secret-particles"><span v-for="i in 10" :key="'4k'+i" class="gc-crystal" /></div>
    <!-- easter: floating colored eggs -->
    <div v-else-if="ach.secret_theme === 'easter'" class="gc-secret-particles"><span v-for="i in 8" :key="'eg'+i" class="gc-egg" /></div>
    <!-- total: sparkles + rotating check -->
    <template v-else-if="ach.secret_theme === 'total'">
      <div class="gc-secret-particles"><span v-for="i in 8" :key="'tot'+i" class="gc-sparkle" /></div>
      <div class="gc-total-check" />
    </template>
    <!-- lonely: blue rain -->
    <div v-else-if="ach.secret_theme === 'lonely'" class="gc-secret-particles"><span v-for="i in 10" :key="'ln'+i" class="gc-raindrop" /></div>
    <!-- triple: triple golden bolts -->
    <div v-else-if="ach.secret_theme === 'triple'" class="gc-triple-bolts"><span v-for="i in 3" :key="'tb'+i" class="gc-bolt" /></div>
    <!-- ultramarathon: embers + rings -->
    <template v-else-if="ach.secret_theme === 'ultramarathon'">
      <div class="gc-secret-particles"><span v-for="i in 8" :key="'um'+i" class="gc-ultraember" /></div>
      <div class="gc-ultra-rings"><span v-for="i in 2" :key="'ur'+i" class="gc-ultra-ring" /></div>
    </template>
    <!-- pipi: water drops -->
    <div v-else-if="ach.secret_theme === 'pipi'" class="gc-secret-particles"><span v-for="i in 8" :key="'pp'+i" class="gc-drop" /></div>
    <!-- zapper: TV static -->
    <div v-else-if="ach.secret_theme === 'zapper'" class="gc-zapper-static" />
    <!-- butterfly: colored wings -->
    <div v-else-if="ach.secret_theme === 'butterfly'" class="gc-secret-particles"><span v-for="i in 6" :key="'bf'+i" class="gc-wing" /></div>
    <!-- bgNoise: zzz + fade -->
    ...
    <!-- sync: linked rings -->
    <div v-else-if="ach.secret_theme === 'sync'" class="gc-sync-rings"><span class="gc-sync-ring gc-sync-ring--1" /><span class="gc-sync-ring gc-sync-ring--2" /></div>
    <!-- countdown: flash -->
    ...
    <div v-else-if="ach.secret_theme === 'gourmet'" class="gc-secret-particles"><span v-for="i in 6" :key="'gm'+i" class="gc-steam" /></div>
    ...
    <template v-else-if="ach.secret_theme === 'ultimateCollector'">
      ...
      <div class="gc-secret-particles"><span v-for="i in 15" :key="'ucp'+i" class="gc-uc-particle" /></div>
