  ...
  <div class="gc-genre-cards-box">
    ...
    <div class="gc-genre-scroll">
      ...
      >
        ...
        <div class="gc-genre-cbar"><div class="gc-genre-cfill" :style="{ width: g.percentage + '%' }" /></div>
        <div class="gc-genre-cpct">{{ g.percentage }}%</div>
        <div v-if="g.count" class="gc-genre-ccount">{{ g.count }} {{ $t('portal.profile.viewedCount') }}</div>
      </div>
    </div>
      ...
      <div class="gc-tile gc-tile--day">
        ...
        <div class="gc-daychart">
          <div v-for="day in weekData" :key="day.label" class="gc-daybar-wrap">
            <div class="gc-daybar">
              <div class="gc-daybar-fill" :style="{ height: day.pct + '%' }" :class="{ 'gc-daybar-fill--top': day.top }" />
            </div>
            <span class="gc-daylabel" :class="{ 'gc-daylabel--top': day.top }">{{ day.label }}</span>
          </div>
        </div>
        <div class="gc-tile-footer"><strong>{{ topDay }}</strong></div>
      </div>
      ...
      <div class="gc-tile gc-tile--ratio">
        ...
        <div v-if="hasMediaRatio" class="gc-ratio">
          <div class="gc-ratio-donut" :style="donutStyle" aria-hidden="true">
            <span class="gc-ratio-donut-inner">{{ moviePct }}<small>%</small></span>
          </div>
    ...
    <div class="gc-tile-row gc-tile-row--duo">
      <div class="gc-tile gc-tile--hours">
        ...
        <div class="gc-hours">
          <div v-for="b in hourBuckets" :key="b.bucket"
               class="gc-hourcol"
               :class="{ 'gc-hourcol--top': b.top }">
            <div class="gc-hourbar"><div class="gc-hourbar-fill" :style="{ height: b.pct + '%' }" /></div>
            <div class="gc-hourlbl">{{ b.label }}</div>
            <div class="gc-hourval">{{ b.count }}</div>
          </div>
        </div>
        <div class="gc-tile-footer"><strong>{{ topHourLabel }}</strong></div>
      </div>
    ...
    <div class="gc-tile-row gc-tile-row--duo">
      <div class="gc-tile gc-tile--rewatch">
        ...
        <div v-if="stats.most_rewatched_movie" class="gc-rewatch">
          <div class="gc-rewatch-poster">
            <img v-if="stats.most_rewatched_movie.poster_url" :src="stats.most_rewatched_movie.poster_url" class="gc-rewatch-poster-img" />
          ...
          <div class="gc-rewatch-info">
            ...
            <div class="gc-rewatch-count">{{ $t('portal.profile.viewedPrefix') }} <strong>{{ stats.most_rewatched_movie.count }} {{ $t('portal.profile.timesSuffix') }}</strong></div>
          </div>
        </div>
        <div v-else class="gc-tile-empty">—</div>
      </div>
      ...
      <div class="gc-tile gc-tile--rewatch">
        ...
        <div v-if="stats.most_rewatched_series" class="gc-rewatch">
          <div class="gc-rewatch-poster">
            <img v-if="stats.most_rewatched_series.poster_url" :src="stats.most_rewatched_series.poster_url" class="gc-rewatch-poster-img" />
            <div v-else class="gc-rewatch-placeholder">📺</div>
          </div>
          <div class="gc-rewatch-info">
            <div class="gc-rewatch-title">{{ stats.most_rewatched_series.title }}</div>
            <div class="gc-rewatch-count"><strong>{{ stats.most_rewatched_series.count }} {{ $t('portal.profile.episodesSuffix') }}</strong> {{ $t('portal.profile.rewatchedSuffix') }}</div>
          </div>
        </div>
        <div v-else class="gc-tile-empty">—</div>
      </div>
...
import {
  CalendarDays, Flame, Film, Clapperboard, Tv, Clock, Trophy,
  Zap, Laugh, Drama, AlertTriangle, Compass, Ghost, Rocket,
  Palette, Sparkles, Users, Camera, Search, Shield, Heart,
...
<style>
@import '@/assets/styles/portal/stats-card.css';
</style>
