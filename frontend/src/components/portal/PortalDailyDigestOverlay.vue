    ...
    <transition name="ddd-fade">
      ...
      >
        ...
        <div ref="panelRef" class="ddd-panel" tabindex="-1">
          ...
          <div v-else-if="error" class="ddd-state ddd-state--error">{{ $t('portal.dailyDigest.errorLoad') }}</div>
          <div v-else-if="!digest || digest.empty" class="ddd-state ddd-state--empty">{{ $t('portal.dailyDigest.empty') }}</div>

          <div v-else class="ddd-body">
            <section v-if="statCards.length" class="ddd-stats" :aria-label="$t('portal.dailyDigest.aria.stats')">
              <div v-for="s in statCards" :key="s.key" class="ddd-stat" :class="s.cls">
                ...
                <div class="ddd-stat-value">
                  <span>{{ s.value }}</span>
                  <span v-if="s.delta" class="ddd-stat-delta" :class="s.deltaClass">{{ s.delta }}</span>
                </div>
            ...
            <section v-if="digest.level" class="ddd-level">
              ...
              <div class="ddd-level-content">
                <span class="ddd-level-eyebrow">{{ $t('portal.dailyDigest.level.eyebrow') }}</span>
                <h2 class="ddd-level-title">{{ $t('portal.profile.titles.' + digest.level.title_key) }}</h2>
                <div class="ddd-level-progress">
                  <div class="ddd-level-progress-fill" :style="{ width: digest.level.percent + '%' }" />
                </div>
                <div class="ddd-level-meta">
                  <span>{{ digest.level.xp_into_level }} / {{ digest.level.xp_next_level - digest.level.xp_current_level }} XP</span>
                  <span class="ddd-level-remaining">{{ $t('portal.dailyDigest.level.toNext', { xp: Math.max(0, digest.level.xp_next_level - digest.level.xp), level: digest.level.level + 1 }) }}</span>
                </div>
              </div>
            </section>

            <section v-if="digest.ranking?.top3?.length" class="ddd-section ddd-section--leaderboard">
            ...
            <section v-if="digest.next_achievement" class="ddd-ach-hero">
              ...
              <div class="ddd-ach-content">
                <span class="ddd-ach-eyebrow">{{ $t('portal.dailyDigest.sections.nextAchievement') }}</span>
                ...
                <div class="ddd-ach-meta">
                  <span class="ddd-ach-count">{{ digest.next_achievement.progress }} / {{ digest.next_achievement.threshold }}</span>
                  <span class="ddd-ach-remaining">{{ $t('portal.dailyDigest.achievement.remaining', { count: digest.next_achievement.remaining }) }}</span>
                </div>
              </div>
            </section>
            ...
            <section v-if="digest.recent_adds.length" class="ddd-section">
              <div class="ddd-section-head">
                <h3 class="ddd-section-title">{{ $t('portal.dailyDigest.sections.recentAdds') }}</h3>
              ...
              <div class="ddd-events">
                <article v-for="ev in digest.events" :key="ev.id" class="ddd-event">
                  ...
                  <div class="ddd-event-body">
                    <span class="ddd-event-kind">{{ $t('portal.dailyDigest.events.' + ev.kind) }}</span>
...
async function dismissToday() {
  try { await apiPost('/api/portal/daily-digest/dismiss', {}) } catch { /* silent: dismiss marker is best-effort */ }
  close()
}
...
function openMedia(item) {
  ...
  close()
  router.push({ name: 'portal-media-detail', params: { type: item.media_type || 'movie', id: item.tmdb_id } })
}
...
watch(
  () => props.open,
  (v) => { if (v) load({ force: true, bypassDismissed: true }) },
)
