    ...
    <div ref="confirmPanelRef" class="mm-modal" role="dialog" aria-modal="true" tabindex="-1">
      ...
      <div class="mm-modal-body">
        <div v-for="n in modalConfirm.lines" :key="n.oldName" class="mm-confirm-line">
          <div class="mm-line-old">
            {{ n.oldName.length > 48 ? '…' + n.oldName.slice(-46) : n.oldName }}
          </div>
          <div class="mm-line-new">
            → {{ n.name.length > 48 ? n.name.slice(0, 46) + '…' : n.name }}
          </div>
        </div>
      </div>
      <div class="mm-modal-footer">
        <button class="mm-btn-sm" @click="modalConfirm.show = false">
          {{ $t('common.cancel') }}
        </button>
  ...
  <div class="mm-overlay" :class="{ show: showRenameErrorsModal }" @click.self="clearRenameErrors">
    ...
    >
      <h3 class="mm-title-error">
        ...
        {{
          $t(
            'mediaManager.renameErrorsTitle',
            { count: renameErrors?.length ?? 0 },
  ...
  <div class="mm-overlay" :class="{ show: showMoveConflictModal }" @click.self="cancelMoveConflict">
    ...
    >
      ...
      <div class="mm-modal-body mm-conflict-body">
        <p class="mm-conflict-intro">
          {{ $t('mediaManager.filesAlreadyExist', { count: moveConflicts.length }) }}
        </p>
      ...
      <div class="mm-modal-footer mm-footer-gap">
        ...
        <button class="mm-btn-sm" @click="execMoveWithOverwrite(false)">
          {{ $t('mediaManager.skipConflictsBtn') }}
        </button>
