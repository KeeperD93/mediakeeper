  ...
  <div class="mm-config-body">
    ...
    <div class="mm-example-block">
      ...
      <div class="mm-example-lines">
        <span>
          HDLight →
          <span class="mm-txt-green">1080p</span>
        </span>
        <span>
          MULTI →
          <span class="mm-txt-green">(vide)</span>
        </span>
        <span>
          \.FRENCH\. →
          <span class="mm-txt-green">.</span>
          <span class="mm-txt-muted">(regex)</span>
        </span>
      </div>
    </div>
      ...
      <div v-for="rule in customRules" :key="rule.id" class="mm-rule-row mm-rule-row-horiz">
        ...
        <div class="mm-rule-text">
          ...
          <div class="mm-rule-hint">
            {{ rule.type === RULE_TYPE.REGEX ? 'Regex' : 'Texte'
            }}{{ rule.flags ? ' · flags: ' + rule.flags : '' }}
          </div>
        </div>
        <button class="mm-btn-sm mm-rule-del" @click="deleteCustomRule(rule.id)">✕</button>
      </div>
      <div v-if="!customRules.length" class="mm-state mm-state-pad">
        <span class="mm-state-text">{{ $t('mediaManager.noRules') }}</span>
      </div>
    </div>
    <hr class="mm-divider" />
    ...
    <div class="mm-add-grid">
      <input
        ...
        class="mm-folder-input mm-input-flat"
    ...
    <div class="mm-regex-row">
      <label class="mm-regex-label">
        <input v-model="newRule.isRegex" type="checkbox" class="mm-chkbox" />
        {{ $t('mediaManager.regexLabel') }}
      </label>
      <input
        v-if="newRule.isRegex"
        v-model="newRule.flags"
        class="mm-folder-input mm-input-flat mm-flags-input"
        placeholder="gi"
      />
    </div>
  </div>
