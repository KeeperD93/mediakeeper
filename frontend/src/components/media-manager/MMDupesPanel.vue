<template>
  <div class="mm-config-body">
    ...
    <button
      ...
      :disabled="!newNames.length || checkingDuplicates"
      @click="checkCrossCatDuplicates"
    >
    ...
    <div v-if="crossCatDuplicates.length" class="mm-dupes-list">
      <div class="mm-label mm-label-error">
        {{ crossCatDuplicates.length }} {{ $t('mediaManager.dupesDetected') }}
      </div>
...
const { newNames, crossCatDuplicates, checkingDuplicates, checkCrossCatDuplicates } =
  useMediaManager()
</script>
