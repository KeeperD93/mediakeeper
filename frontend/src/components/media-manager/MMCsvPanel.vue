  ...
  <div class="mm-config-body">
    ...
    <div class="mm-action-row mm-action-row-top">
      <button
        ...
        @click="exportRenameCsv(newNames)"
      >
        <Download :size="12" />
        <span v-if="useCountSuffix">
          {{ $t('mediaManager.exportCsvCount', { count: newNames.length }) }}
        </span>
    ...
    <div v-if="csvPreview.length" class="mm-preview-block">
      ...
      <div class="mm-preview-scroll">
        <div
          v-for="(row, ri) in csvPreview.slice(0, 8)"
          :key="ri"
          class="mm-hist-item mm-preview-row"
        >
...
const { csvDragOver, csvPreview, dropCsv, pickCsvFile, applyCsvImport, downloadCsvTemplate } =
  useMMConfigPanels()
</script>
