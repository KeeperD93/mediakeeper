<template>
  <div class="mm-overlay" :class="{show: showAdvancedModal}" @click.self="showAdvancedModal=false">
    <div class="mm-config-modal mm-config-modal-760">
      <div class="mm-config-sidebar">
        ...
        <button v-for="tab in ADVANCED_TABS" :key="tab.id" class="mm-config-tab" :class="{active: advancedTab === tab.id}" @click="advancedTab = tab.id">
      ...
      <div class="mm-config-content">
        <div class="mm-config-header">
          ...
          <button class="mm-btn-sm mm-close-btn" type="button" :aria-label="$t('common.close')" @click="showAdvancedModal=false"><X :size="14" /></button>
        </div>
        ...
        <div v-if="advancedTab==='profiles'" class="mm-config-body">
          ...
          <div class="mm-adv-profiles">
            <div v-for="p in getAllProfiles()" :key="p.id" class="mm-adv-profile-card" :class="{builtin: p.builtin}">
              <div class="mm-adv-profile-name">{{ p.name }}</div>
              <div class="mm-adv-profile-fmt">
                <span class="mm-rf-tag mm-rf-tag-sm">{{ $t('common.film') }}</span> {{ p.config.movie }}<br>
                <span class="mm-rf-tag mm-rf-tag-sm">{{ $t('common.series') }}</span> {{ p.config.tv }}
              </div>
              <div class="mm-adv-profile-actions">
                <button class="mm-btn-sm mm-btn-accent" @click="applyProfileLocal(p)">{{ $t('common.apply') }}</button>
                <button v-if="!p.builtin" class="mm-btn-sm mm-btn-del" @click="deleteProfile(p.id)">✕</button>
              </div>
            </div>
          </div>
          <hr class="mm-divider"/>
          ...
          <div class="mm-save-row">
            <input v-model="newProfileName" class="mm-folder-input mm-input-flat mm-input-flex" :placeholder="$t('mediaManager.profileNamePlaceholder')" @keydown.enter="saveCurrentAsProfile"/>
            <button class="mm-btn-sm mm-btn-success" @click="saveCurrentAsProfile">{{ $t('mediaManager.saveBtnLabel') }}</button>
          </div>
        </div>
...
const advancedTabLabel = computed(() => ADVANCED_TABS.find(t => t.id === advancedTab.value)?.label || '')
</script>

<style scoped>
/* Modal width */
.mm-config-modal-760 { width: 760px; }
/* Close button (header ×) */
...
.mm-rf-tag-sm { font-size: 0.6rem; }
/* Delete (red) button inside the profile card */
.mm-btn-del { color: var(--mm-red); }
/* Divider between profiles list and "save profile" form */
.mm-divider { border-color: var(--border); margin: .8rem 0; }
/* Save-profile row (input + button) */
.mm-save-row { display: flex; gap: .5rem; margin-top: .4rem; }
