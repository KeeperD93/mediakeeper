  ...
  <div>
      ...
      <div class="backup-dest-row">
        <div class="backup-dest-input-wrap">
          ...
          <input
            ...
            @keydown.enter="changeBackupDir(backupDirInput)"
          />
          <button
            class="backup-dest-save"
            :disabled="backupDirInput === (backupInfo?.backup_dir || '/data/backups')"
            @click="changeBackupDir(backupDirInput)"
          >
            {{ $t('common.save') }}
          </button>
        </div>
    ...
    <section class="params-section">
      ...
      <div class="backup-retention-row">
        <select
          ...
          :value="retentionMode"
          @change="onRetentionModeChange($event.target.value)"
        >
        ...
        <button class="backup-retention-save" @click="saveRetention">
          {{ $t('common.save') }}
        </button>
      </div>
    </section>
    ...
    <section class="params-section">
      <h3 class="params-section-title">
        {{ $t('backup.history') }} ({{ backupInfo?.backups?.length || 0 }})
      </h3>
