<template>
  <div class="hero-wrap" :class="{ 'hero-entered': entered }">
    ...
    <template v-else>
      <div
        class="hero-backdrop-layer"
        :class="{ visible: showA, 'kb-a': true }"
        :style="{ backgroundImage: backdropA ? `url(${backdropA})` : 'none' }"
      />
      <div
        class="hero-backdrop-layer"
        :class="{ visible: !showA, 'kb-b': true }"
...
const ambianceStyle = computed(() => {
  for (const gid of current.value.genre_ids || []) {
...
function restartTimer() {
  ...
  if (props.sessions.length > 1)
    timer = setInterval(() => {
      idx.value = (idx.value + 1) % props.sessions.length
      posterError.value = false
    }, 10000)
}
...
watch(
  ...
  () => {
    if (idx.value >= props.sessions.length) idx.value = 0
    restartTimer()
  },
)
watch(
...
onMounted(() => {
  ...
  if (currentPoster.value) backdropA.value = currentPoster.value
...
.hero-wrap:not(:has(.hero-content)) .hero-backdrop-layer {
