    ...
    <div class="logs-content">
      ...
      <template v-if="!currentFile">
        ...
        <div v-else class="logs-files">
          <div v-for="f in files" :key="f.filename" class="logs-file-card" @click="viewFile(f.filename)">
            <div class="file-icon" :class="f.filename.startsWith('emby') ? 'icon-emby' : 'icon-mk'">
      ...
      <template v-else>
        <div class="reader-toolbar">
          ...
          <div class="reader-actions">
            <button class="reader-btn" :class="{ active: autoRefresh }" @click="toggleAutoRefresh">
              <RefreshCw :size="13" />
        ...
        <div class="reader-filters">
          <div class="level-buttons">
            <button v-for="lvl in levels" :key="lvl.name" class="level-btn" :class="{ active: filters[lvl.name] }" :style="lvl.style" @click="filters[lvl.name] = !filters[lvl.name]">
              {{ lvl.name }}
            </button>
          </div>
          ...
          <input v-model="search" type="text" :placeholder="$t('logs.searchPlaceholder')" class="reader-search" />
