    ...
    <div class="nf-content">
      ...
      <div v-show="activeTab === 'agents'" class="tab-panel">
        <div class="nf-kpis">
          <div class="nf-kpi glass-kpi">
            <span class="kpi-val" :class="discord.enabled ? 'nf-kpi-on' : 'nf-kpi-off'">
        ...
        <div class="nf-agent-bar">
          <button
            ...
            @click="agent = a.id"
          >
            <component :is="a.icon" class="nf-agent-icon" />
            {{ a.label }}
          </button>
        </div>
        ...
        <template v-if="agent === 'discord'">
          <div class="nf-section">
            <div class="nf-row">
              <label class="nf-switch nf-switch-row">
                <input v-model="discord.enabled" type="checkbox" @change="saveDiscord" />
                <div class="sw-track" />
                <span class="nf-switch-label">{{ $t('notifications.discord.enable') }}</span>
              </label>
              <label class="nf-small-label">
                {{ $t('notifications.discord.delaySec') }}
                <input
                  v-model.number="discord.delay"
                  type="number"
                  min="0"
                  max="300"
                  class="nf-input-sm"
          ...
          <div class="nf-section">
            <div class="nf-section-hdr">
              <h3 class="nf-section-title">{{ $t('notifications.discord.webhooks') }}</h3>
              <button class="nf-add-btn" @click="addWebhook">
                {{ $t('notifications.discord.addWebhook') }}
              </button>
            </div>
            <MkEmptyState
              v-if="!discord.webhooks.length"
              size="sm"
              :title="$t('notifications.discord.noWebhooks')"
            />
            <div v-for="(wh, idx) in discord.webhooks" :key="wh.id" class="nf-wh">
              <div
                class="nf-wh-head"
                :class="{ disabled: !wh.enabled }"
                @click="wh._open = !wh._open"
              >
                ...
                <span class="nf-wh-status" :class="wh.enabled ? 'on' : 'off'">
                  {{ wh.enabled ? $t('common.active') : $t('common.off') }}
                </span>
                <ChevronDown class="nf-chevron" :class="{ open: wh._open }" />
              </div>
              <div v-if="wh._open" class="nf-wh-body">
                <div class="nf-wh-top">
                  <div class="nf-field nf-field-flex">
                    <label class="nf-label">
                      {{ $t('notifications.discord.webhookNameLabel') }}
                    </label>
                    <input
                      v-model="wh.name"
                      type="text"
                      class="nf-input nf-input-bold"
                      :placeholder="$t('notifications.discord.webhookNamePlaceholder')"
                    />
                  </div>
                  <div class="nf-wh-ctrls">
                    <label class="nf-switch">
                      <input v-model="wh.enabled" type="checkbox" @change="saveDiscord" />
                      <div class="sw-track" />
                    </label>
                    <button
                      class="nf-del-btn"
                      @click="(discord.webhooks.splice(idx, 1), saveDiscord())"
                    >
                      {{ $t('common.delete') }}
                    </button>
                  </div>
                ...
                <div class="nf-field">
                  <label class="nf-label">{{ $t('notifications.discord.urlLabel') }}</label>
                  <input
                    v-model="wh.url"
                    type="text"
                    class="nf-input"
                    :placeholder="
                      wh.url_configured && !wh.url
                        ? 'Deja configuree - saisir une nouvelle URL pour remplacer'
                ...
                <div class="nf-field">
                  ...
                  <div class="nf-events">
                    <label v-for="ev in eventList" :key="ev.key" class="nf-ev-chk">
                      <input v-model="wh.events[ev.key]" type="checkbox" />
                      <span>{{ ev.label }}</span>
                    </label>
                  </div>
                </div>
                <div class="nf-test-row">
                  <span class="nf-test-label">
                    {{ $t('notifications.discord.testLabelShort') }}
                  </span>
                  <button
                    v-for="tt in testTypes"
                    :key="tt.id"
                    class="nf-test-btn"
                    :disabled="testing === idx + tt.id"
                    @click="testWh(idx, tt.id)"
                  >
                    {{ testing === idx + tt.id ? '...' : tt.label }}
                  </button>
                </div>
              </div>
            </div>
          ...
          <button class="nf-save-btn" :disabled="saving" @click="saveDiscord">
            {{ saving ? $t('common.saving') : $t('notifications.discord.saveDiscord') }}
          </button>
        </template>
        <template v-else>
          <MkEmptyState size="sm" :title="$t('notifications.discord.comingSoon', { agent })" />
        </template>
      </div>
        ...
        <div class="nf-rules-grid">
          <div class="nf-section">
            ...
            <div v-if="rules.dnd_enabled" class="nf-dnd-times">
              <div class="nf-field">
                <label class="nf-label">{{ $t('notifications.discord.dndFrom') }}</label>
                <input v-model="rules.dnd_start" type="time" class="nf-input nf-input-w130" />
              </div>
              <span class="nf-dnd-arrow">→</span>
              <div class="nf-field">
                <label class="nf-label">{{ $t('notifications.discord.dndTo') }}</label>
                <input v-model="rules.dnd_end" type="time" class="nf-input nf-input-w130" />
              </div>
            </div>
          </div>
          <div class="nf-section">
            ...
            <div class="nf-field">
              ...
              <input
                ...
                type="text"
                class="nf-input"
                :placeholder="$t('notifications.discord.librariesPlaceholder')"
              />
            </div>
            <div class="nf-field">
              <label class="nf-label">{{ $t('notifications.discord.excludedGenres') }}</label>
              <input
                v-model="genreFilterText"
                type="text"
                class="nf-input"
                :placeholder="$t('notifications.discord.genresPlaceholder')"
              />
            </div>
          </div>
        </div>
        <button class="nf-save-btn nf-save-btn-spaced" @click="saveRules">
          {{ $t('notifications.discord.saveRules') }}
        </button>
      </div>
      ...
      <div v-show="activeTab === 'history'" class="tab-panel">
        <div class="nf-hist-bar">
          <select v-model="histFilter" class="nf-select mk-select-chevron nf-select-w140">
            ...
            <option value="grouped">{{ $t('notifications.discord.historyGrouped') }}</option>
          </select>
          <button class="nf-btn-sm nf-btn-danger" @click="clearHistory">
            {{ $t('notifications.discord.clearHistory') }}
          </button>
        </div>
        <MkEmptyState
          v-if="!filteredHistory.length"
          size="sm"
          :title="$t('notifications.discord.noHistory')"
        />
        <div v-else class="nf-hist-list">
          <div v-for="h in filteredHistory" :key="h.id" class="nf-hist-row">
            ...
            <div class="nf-hist-meta">
              ...
              <span v-if="h.error" class="nf-hist-err" :title="h.error">
                {{ $t('notifications.discord.historyError') }}
              </span>
            </div>
          </div>
        </div>
      ...
      <div v-show="activeTab === 'config'" class="tab-panel">
        ...
        <div class="nf-section">
          ...
          <div class="nf-field">
            <label class="nf-label">{{ $t('notifications.discord.imgurClientId') }}</label>
            <input v-model="imgur.client_id" type="text" class="nf-input" placeholder="Client ID" />
          </div>
          <div class="nf-field">
            <label class="nf-label">{{ $t('notifications.discord.imgurClientSecret') }}</label>
            <input
              :type="imgurSecretInputType"
              :value="displayImgurSecret"
              class="nf-input"
              placeholder="Client Secret"
              @focus="onImgurSecretFocus"
              @input="onImgurSecretInput($event.target.value)"
            />
          </div>
          <button class="nf-save-sm" @click="saveImgurMasked">
            {{ $t('notifications.discord.saveImgur') }}
          </button>
        </div>
      </div>
    </div>
...
const imgurSecretMask = computed(() =>
  '*'.repeat(Math.max(1, Number(imgur.client_secret_length) || 0)),
)
...
const eventList = computed(() => [
  { key: 'added', label: t('notifications.events.added') },
  { key: 'offline', label: t('notifications.events.offline') },
  { key: 'duplicate', label: t('notifications.events.duplicate') },
  { key: 'new_request', label: t('notifications.events.new_request') },
  { key: 'request_approved', label: t('notifications.events.request_approved') },
  { key: 'request_available', label: t('notifications.events.request_available') },
  { key: 'partially_available', label: t('notifications.events.partially_available') },
  { key: 'request_rejected', label: t('notifications.events.request_rejected') },
  { key: 'new_issue', label: t('notifications.events.new_issue') },
  { key: 'issue_comment', label: t('notifications.events.issue_comment') },
  { key: 'issue_resolved', label: t('notifications.events.issue_resolved') },
  { key: 'emby_alerts', label: t('notifications.events.emby_alerts') },
])
</script>
