    ...
    <div class="doub-content">
      ...
      <div v-show="activeTab==='duplicates'" class="tab-panel">
        ...
        <div class="doub-kpis">
          ...
          <div class="doub-kpi glass-kpi">
            <span class="kpi-val doub-kpi-info">{{ formatBytes(historyStats.total_bytes_freed) }}</span>
        ...
        <div class="doub-header">
          <div class="doub-actions">
            ...
            <button v-if="activeDuplicates.length && rules.length" class="doub-btn doub-btn-accent" @click="applyRules">
              <Zap class="ic" />
        ...
        <div v-if="loading && !duplicates.length" class="doub-grid">
          <div v-for="i in 3" :key="'sk'+i" class="doub-card doub-skeleton">
            <div class="skel-poster"/><div class="skel-body"><div class="skel-line skel-title"/><div class="skel-line skel-sub"/><div class="skel-source"/></div>
          </div>
        </div>
        <MkEmptyState
        ...
        <div v-else class="doub-grid">
          <div v-for="item in activeDuplicates" :key="item.id" class="doub-card">
            <div class="doub-poster">
              <img v-if="item.poster" :src="item.poster" class="doub-poster-img" @error="e=>e.target.style.display='none'"/>
            ...
            <div class="doub-info">
              <div class="doub-info-header">
                <div>
                  ...
                  <p class="doub-versions">{{ item.sources.length }} {{ $t('duplicates.versions') }} · {{ reclaimableFor(item) }} {{ $t('duplicates.reclaimable') }}</p>
                </div>
                <div class="doub-info-btns">
                  <button v-if="bestSource(item)" class="doub-suggest-btn" :title="$t('duplicates.keepBest')" @click="keepSource(item, bestSource(item))">
                    <Zap class="ic-sm" />
                    {{ $t('duplicates.keepBestBtn') }}
                  </button>
                  <button class="doub-compare-btn" :class="{active:compareOpen===item.id}" @click="toggleCompare(item.id)">
                    <Files class="ic-sm" />
              ...
              <div v-if="compareOpen===item.id" class="doub-compare">
                <table class="cmp-table">
                  <thead><tr><th>{{ $t('duplicates.file') }}</th><th>{{ $t('duplicates.resolution') }}</th><th>{{ $t('duplicates.codec') }}</th><th>{{ $t('duplicates.size') }}</th><th>{{ $t('duplicates.score') }}</th><th></th></tr></thead>
                  <tbody>
                    <tr v-for="(src,si) in item.sources" :key="si" :class="{'cmp-best':isBest(item,src)}">
                      <td class="cmp-file" :title="src.name">{{ src.name }}</td>
                      <td>{{ src.resolution }}</td>
                      <td>{{ src.codec }}</td>
                      <td>{{ src.size_label }}</td>
                      <td><span class="cmp-score" :style="{color:scoreColor(srcScore(src))}">{{ srcScore(src) }}</span></td>
                      <td class="cmp-actions">
                        <button class="doub-keep-btn" @click="keepSource(item,src)">{{ $t('duplicates.keep') }}</button>
                        <button class="doub-delete-btn" @click="deleteSource(item,src)">{{ $t('duplicates.delete') }}</button>
                      </td>
                    </tr>
                  </tbody>
                </table>
              ...
              <div v-else class="doub-sources">
                <div v-for="(src,si) in item.sources" :key="si" class="doub-source" :class="{'doub-source-best':isBest(item,src)}">
                  <div class="doub-source-info">
                    ...
                    <div class="doub-tags">
                      ...
                      <span v-if="isBest(item,src)" class="doub-tag doub-tag-best">★ {{ $t('duplicates.best') }}</span>
                    </div>
                  </div>
                  <div class="doub-source-acts">
                    <button class="doub-keep-btn" @click="keepSource(item,src)">{{ $t('duplicates.keep') }}</button>
                    <button class="doub-delete-btn" @click="deleteSource(item,src)">{{ $t('duplicates.deleteFile') }}</button>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="doub-info doub-info-flex">
              <div class="doub-info-header">
                <div>
                  ...
                  <p v-if="ig.ignored_at" class="doub-versions">{{ $t('duplicates.ignoredOn', { date: fmtDate(ig.ignored_at) }) }}</p>
                </div>
      ...
      <div v-show="activeTab==='rules'" class="tab-panel">
        ...
        <div class="rules-list">
          <div v-for="(rule,ri) in rules" :key="ri" class="rule-row">
            ...
            <input v-if="rule.field==='resolution'" v-model="rule.value" class="rule-input" placeholder="ex: 1080p"/>
            <input v-else-if="rule.field==='codec'" v-model="rule.value" class="rule-input" placeholder="ex: HEVC"/>
            <span v-else class="rule-auto">{{ $t('duplicates.automatic') }}</span>
            <button class="rule-del" @click="rules.splice(ri,1);saveRules()">✕</button>
          </div>
        </div>
        <button class="doub-btn doub-btn-secondary doub-btn-spaced" @click="rules.push({field:'keep_largest',value:''});saveRules()">
...
import { CircleCheck, Check, EyeOff, Film, Files, RefreshCw, Search, Trash2, Zap } from 'lucide-vue-next'
...
function toggleCompare(id) { compareOpen.value = compareOpen.value === id ? null : id }

onMounted(async () => { await Promise.all([loadDuplicates(), loadIgnored(), loadHistory(), loadHistoryStats()]) })
</script>
