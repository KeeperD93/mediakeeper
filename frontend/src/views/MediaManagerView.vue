  ...
  <div class="cinema-mm mk-page-root">
    <div class="mm-content">
      ...
      <div class="mm-main">
        ...
        <MMTmdbPanel ref="tmdbPanelComp" @open-config="openConfig" />
...
function onKeyDown(e) {
  ...
  if (e.key === 'Delete') {
    ...
    if (sel.length === 1) deleteFile(sel[0])
    else if (sel.length > 1) deleteSelected()
    ...
    if (subPath.value) navRoot()
    else checked.value = new Set()
  } else if (e.key === 'ArrowLeft') {
    const idx = CATS.value.findIndex(c => c.key === activeCat.value)
    if (idx > 0) setCat(CATS.value[idx - 1].key)
  } else if (e.key === 'ArrowRight') {
    const idx = CATS.value.findIndex(c => c.key === activeCat.value)
    if (idx < CATS.value.length - 1) setCat(CATS.value[idx + 1].key)
  }
}
...
function purgeOldHistory() {
  ...
  if (renameHistory.value.length !== before)
    try {
      localStorage.setItem('mk_rename_history', JSON.stringify(renameHistory.value))
    } catch {
      /* silent: localStorage quota/privacy mode → skip persist */
    }
}
