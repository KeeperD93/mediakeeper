  ...
  <div class="pt-parties">
    <div class="pt-parties-header">
      ...
      <button class="pt-btn pt-btn--primary" @click="showCreate = true">
        <i class="icon-plus" /> {{ $t('portal.parties.create') }}
      </button>
    </div>
    ...
    <div class="pt-parties-grid">
      <div v-for="p in parties" :key="p.id" class="pt-party-card">
        <div class="pt-party-date">
          {{ new Date(p.scheduled_at).toLocaleDateString(undefined, { weekday: 'short', month: 'short', day: 'numeric' }) }}
          <br />
        ...
        <div class="pt-party-info">
          ...
          <span class="pt-party-meta">
            {{ p.participant_count }}/{{ p.max_participants }} {{ $t('portal.parties.participants') }}
          </span>
        </div>
      ...
      <div class="pt-popup-overlay" @click.self="showCreate = false">
        <div class="pt-popup pt-popup--md">
          <div class="pt-popup-header">
            ...
            <button class="pt-popup-close" type="button" :aria-label="$t('common.close')" @click="showCreate = false"><X :size="14" /></button>
          </div>
          <div class="pt-popup-body">
            ...
            <input v-model.number="form.max_participants" type="number" min="2" max="100" class="pt-input" />
          </div>
...
<style scoped>
.pt-parties { max-width: 900px; margin: 0 auto; padding: 1.5rem; }
.pt-parties-header { display: flex; align-items: center; justify-content: space-between; margin-bottom: 1.25rem; }
.pt-parties-header h2 { font-size: var(--portal-text-xl); font-weight: var(--portal-font-bold); color: var(--text-primary); }
.pt-parties-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 1rem; }
.pt-party-card { display: flex; align-items: center; gap: 1rem; padding: 1rem; background: var(--bg-secondary); border-radius: var(--radius-card); border: 1px solid var(--border); }
.pt-party-date { text-align: center; font-size: var(--portal-text-xs); font-weight: var(--portal-font-bold); color: var(--accent); min-width: 60px; }
.pt-party-info { flex: 1; }
.pt-party-info h3 { font-size: var(--portal-text-base); font-weight: var(--portal-font-bold); color: var(--text-primary); }
.pt-party-meta { font-size: var(--portal-text-xs); color: var(--text-muted); }
.pt-btn { padding: 0.45rem 1rem; border-radius: var(--radius-btn); border: none; font-weight: var(--portal-font-medium); cursor: pointer; font-size: var(--portal-text-xs); display: inline-flex; align-items: center; gap: 0.3rem; }
.pt-btn--primary { background: var(--accent); color: #fff; }
.pt-btn--secondary { background: var(--bg-tertiary); color: var(--text-secondary); border: 1px solid var(--border); }
.pt-empty { color: var(--text-muted); text-align: center; padding: 2rem 0; grid-column: 1/-1; }
...
.pt-popup-body label { display: block; font-size: var(--portal-text-xs); color: var(--text-muted); margin: 0.75rem 0 0.25rem; }
