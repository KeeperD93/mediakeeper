<template>
  <div ref="rootRef" class="cinema-health mk-page-root">
    ...
    <div v-if="scanStatus.running" class="hc-progress-wrap">
      <div class="hc-progress-bar"><div class="hc-progress-fill" :style="{ width: progressPct + '%' }" /></div>
      <span class="hc-progress-text">{{ scanStatus.progress }} / {{ scanStatus.total }} — {{ scanStatus.current_item }}</span>
    </div>
    ...
    <div v-show="tab === 'health'" class="tab-panel">
      ...
      <div class="hc-header">
        <span v-if="summary && summary.last_scan" class="hc-last-scan">{{ $t('healthCheck.lastScan') }} {{ formatAgo(summary.last_scan) }}</span>
      ...
      <div v-if="summary" class="hc-hero-row">
        <div class="hc-score-card glass-card">
          <svg viewBox="0 0 120 120" class="hc-score-ring">
            ...
            <circle cx="60" cy="60" r="52" fill="none" :style="{ stroke: scoreColor }" stroke-width="8"
              stroke-linecap="round" :stroke-dasharray="scoreCircum" :stroke-dashoffset="scoreOffset"
              transform="rotate(-90 60 60)" class="hc-score-arc" />
          </svg>
        ...
        <div class="hc-severity-col">
          <div class="glass-card hc-sev-card" :class="{ 'hc-sev-active': filterSev === 'critical' }" @click="toggleSeverity('critical')">
            <span class="hc-sev-dot hc-sev-dot-critical" />
            <div class="hc-sev-text"><span class="hc-sev-count hc-sev-count-critical">{{ summary.critical }}</span><span class="hc-sev-label">{{ $t('healthCheck.critical') }}</span></div>
          </div>
          <div class="glass-card hc-sev-card" :class="{ 'hc-sev-active': filterSev === 'warning' }" @click="toggleSeverity('warning')">
            <span class="hc-sev-dot hc-sev-dot-warning" />
            <div class="hc-sev-text"><span class="hc-sev-count hc-sev-count-warning">{{ summary.warning }}</span><span class="hc-sev-label">{{ $t('healthCheck.warning') }}</span></div>
          </div>
          <div class="glass-card hc-sev-card" :class="{ 'hc-sev-active': filterSev === 'info' }" @click="toggleSeverity('info')">
            <span class="hc-sev-dot hc-sev-dot-info" />
            <div class="hc-sev-text"><span class="hc-sev-count hc-sev-count-info">{{ summary.info }}</span><span class="hc-sev-label">{{ $t('healthCheck.info') }}</span></div>
          </div>
        </div>
      </div>
      ...
      <div v-if="summary" class="hc-breakdown-row">
        <div v-if="filteredLibraries.length" class="glass-card hc-breakdown-card">
          ...
          <div class="hc-lib-list">
            <div v-for="[lib, count] in filteredLibraries" :key="lib" class="hc-lib-item" :class="{ 'hc-lib-active': filterLib === lib }" @click="toggleLibrary(lib)">
              <div class="hc-lib-row">
                <span class="hc-lib-name">{{ lib }}</span>
                <span class="hc-lib-count">{{ count }}</span>
              </div>
              <div class="hc-lib-bar"><div class="hc-lib-bar-fill" :style="{ width: libPct(count) + '%' }" /></div>
            </div>
          </div>
        </div>
        <div v-if="Object.keys(summary.by_type || {}).length" class="glass-card hc-breakdown-card">
          ...
          <div class="hc-types-wrap">
            <div v-for="(count, typ) in summary.by_type" :key="typ" class="hc-type-chip" :class="{ 'hc-type-active': filterTyp === typ }" @click="toggleType(typ)">
          ...
          <template v-if="Object.keys(summary.by_extension || {}).length">
            <h3 class="hc-section-title hc-section-title-spaced">{{ $t('healthCheck.byExtension') }}</h3>
            <div class="hc-types-wrap">
              <div v-for="(count, ext) in summary.by_extension" :key="ext" class="hc-type-chip hc-ext-chip" :class="{ 'hc-type-active': filterExt === ext }" @click="toggleExt(ext)">
      ...
      <div v-if="summary && summary.total_issues > 0" class="glass-card hc-issues-card">
        <div class="hc-issues-header">
          <h3 class="hc-section-title">{{ $t('healthCheck.detectedIssues') }} <span class="hc-issues-total">({{ issues.total }})</span></h3>
          <div class="hc-filters">
            ...
            <select v-if="libOptions.length" v-model="filterLib" class="hc-select" @change="reloadAll()">
        ...
        <template v-if="groupedPosters.length">
          <div class="hc-poster-grid">
            <div v-for="item in groupedPosters" :key="item.key" class="sg-card" @click="selectedIssue = item">
...
watch(() => issues.value.items?.length, () => {
  nextTick(() => { if (sentinelRef.value && observer) observer.observe(sentinelRef.value) })
})
