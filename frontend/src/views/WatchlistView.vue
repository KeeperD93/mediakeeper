<template>
  <div class="cinema-wl mk-page-root">
    ...
    <div class="wl-inner">
      ...
      <template v-else>
        <div v-show="activeTab === 'missing'" class="tab-panel"><WlSeriesView type="missing" @scan="refreshScan" /></div>
        <div v-if="mounted.timeline" v-show="activeTab === 'timeline'" class="tab-panel"><WlTimelineView /></div>
        <div v-show="activeTab === 'suivi'" class="tab-panel"><WlSuiviView /></div>
        <div v-if="mounted.calendar" v-show="activeTab === 'calendar'" class="tab-panel"><WlCalendarView /></div>
...
const WlTimelineView = defineAsyncComponent(() => import('@/components/watchlist/WlTimelineView.vue'))
const WlSuiviView = defineAsyncComponent(() => import('@/components/watchlist/WlSuiviView.vue'))
const WlCalendarView = defineAsyncComponent(() => import('@/components/watchlist/WlCalendarView.vue'))
...
const { data, loading, loadIgnored, loadTracked, loadScan, refreshScan, prefetchCalendar } = useWatchlist()
...
<style scoped>
.cinema-wl { position:relative; min-height:100%; padding:12px 24px 24px; overflow:hidden; }
.wl-inner { position:relative; z-index:1; }
.tab-panel { animation:tab-in .25s ease; }
@keyframes tab-in { from{opacity:0;transform:translateY(6px)} to{opacity:1;transform:translateY(0)} }
.wl-center { display:flex; flex-direction:column; align-items:center; justify-content:center; padding:80px 24px; gap:16px; }
.wl-scan-btn { display:inline-flex; align-items:center; gap:6px; padding:10px 20px; border-radius:var(--radius-card); background:var(--accent-600); color:#fff; font-size:var(--text-sm); font-weight:var(--font-medium); border:none; cursor:pointer; font-family:inherit; }
.wl-empty-icon { opacity:0.2; color:var(--text-muted); }
.wl-empty-text { color:var(--text-muted); font-size:var(--text-sm); }
@media(max-width:767px) { .cinema-wl{padding:0 16px 16px} }
</style>
