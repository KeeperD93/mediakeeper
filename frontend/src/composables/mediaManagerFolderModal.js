...
function _stripAudioSuffix(name) {
  const re = new RegExp(`\\s*\\((?:${_ALL_AUDIO_TAGS.join('|')})(?:\\s+(?:${_ALL_AUDIO_TAGS.join('|')}))*\\)\\s*$`, 'i')
...
export function openFolderModal() {
  ...
  folderExistingDirs.value = files.value.filter(f => f.type === FILE_TYPE.FOLDER).map(d => {
    const sNum = detectSeasonNum(d.name)
    return { name: d.name, path: d.path, newName: sNum !== null ? `Saison ${String(sNum).padStart(2, '0')}` : d.name }
  })
  autoDetectFolders()
  updateFolderPreview()
...
export function updateFolderPreview() {
  ...
  active.forEach(s => { p += `  └── 📁 Saison ${String(s.season).padStart(2, '0')}${suf}\n` })
...
export async function execFolderOrganize() {
  ...
  if (!seriesName) { showToast(_t('mediaManager.seriesNameMissing'), TOAST_TYPE.ERR); return }
  ...
  let actualSeriesPath = subPath.value ? `${catBase.split('/').slice(0, -subPath.value.split('/').length).join('/')}/${seriesName}` : `${catBase}/${seriesName}`
  if (subPath.value) {
    const currentFolderName = subPath.value.split('/').pop()
    if (currentFolderName !== seriesName) {
      try {
        const rfRes = await apiFetch('/api/media/rename-folder', { method: 'POST', body: JSON.stringify({ cat: activeCat.value, subpath: subPath.value, new_name: seriesName }) })
  ...
  if (active.length > 0) {
    await apiFetch('/api/media/create-folders', {
      method: 'POST',
      body: JSON.stringify({ folders: active.map(s => ({ parent_path: actualSeriesPath, folder_name: `Saison ${String(s.season).padStart(2, '0')}${audioSuffix}` })) }),
    })
  }
  endProgress()
  const msg = subPath.value && active.length ? _t('mediaManager.folderRenamedWithSeasons', { count: active.length })
            : subPath.value ? _t('mediaManager.folderRenamedSimple')
            : active.length ? _t('mediaManager.seasonsCreated', { count: active.length })
            : _t('mediaManager.done')
  showToast(msg, TOAST_TYPE.OK)
  if (subPath.value) {
    const currentFolderName = subPath.value.split('/').pop()
    if (currentFolderName !== seriesName) {
      subPath.value = subPath.value.split('/').slice(0, -1).concat([seriesName]).join('/').replace(/^\//, '')
    }
  }
  loadFiles()
}
...
export async function execRenameAllFolders() {
  ...
  if (!toRename.length) { showToast(_t('mediaManager.noFoldersToRename'), TOAST_TYPE.ERR); return }
  setProgress(5)
  let catBase = CATS.value.find(c => c.key === activeCat.value)?.path || ''
  if (files.value.length > 0 && files.value[0].path) {
    const firstPath = files.value[0].path
  ...
  for (let i = 0; i < toRename.length; i++) {
    ...
    try {
      const res = await apiFetch('/api/media/rename-folder', { method: 'POST', body: JSON.stringify({ cat: activeCat.value, subpath: relPath, new_name: newName }) })
      const data = await res.json()
      if (data.error) err++
      else { ok++; if (data.merged) merged++ }
    } catch { err++ }
  }
  endProgress()
  ...
  folderExistingDirs.value = folderExistingDirs.value.map(d => {
    ...
    if (renamed) return { ...d, name: renamed.newName, path: d.path.replace(/[^/]+$/, renamed.newName) }
    return d
  })
  loadFiles()
}
