import {
  apiFetch, showToast, _t,
  activeCat, filtered, checked, newNames, dragSrc,
  modalConfirm, expandedMode,
  renameHistory, _saveRenameHistory, fileRenameStatus,
...
function _linkOrphansToCheckedSources() {
  const orphans = []
  newNames.value.forEach((n, i) => { if (!n.path) orphans.push(i) })
...
export function clearRenameErrors() { renameErrors.value = []; showRenameErrorsModal.value = false }

export async function execRename() {
  modalConfirm.value.show = false
  const toRename = newNames.value.filter(n => n.path && n.path !== '')
  if (!toRename.length) { showToast(_t('mediaManager.noFilesToRename'), TOAST_TYPE.ERR); return }
  const statusMap = {}
  toRename.forEach(n => { statusMap[n.path] = 'pending' })
  ...
  for (let i = 0; i < toRename.length; i++) {
    ...
    try {
      const res = await apiFetch('/api/media/rename-batch', { method: 'POST', body: JSON.stringify({ items: [{ old_path: n.path, new_name: n.name }], cat: activeCat.value }) })
      const results = await res.json()
      if (results[0]?.success) {
        fileRenameStatus.value = { ...fileRenameStatus.value, [n.path]: 'done' }
        historyItems.push({ oldName: n.oldName, newName: n.name, path: n.path, newPath: results[0].new_path || '' })
        applyRenameInPlace(n.path, n.name, results[0].new_path)
        ok++
      } else {
        fileRenameStatus.value = { ...fileRenameStatus.value, [n.path]: 'error' }
        errors.push({ oldName: n.oldName, newName: n.name, path: n.path, reason: results[0]?.error || results[0]?.message || _t('common.unknownError') })
        errCount++
      }
    } catch (e) {
      fileRenameStatus.value = { ...fileRenameStatus.value, [n.path]: 'error' }
      errors.push({ oldName: n.oldName, newName: n.name, path: n.path, reason: e?.message || _t('common.networkError') })
      errCount++
    }
  }
  if (historyItems.length > 0) {
    renameHistory.value.unshift({ timestamp: Date.now(), cat: activeCat.value, tmdbTitle: selectedTmdb.value?.title || '', items: historyItems })
    _saveRenameHistory()
  }
  endProgress()
  setTimeout(() => { fileRenameStatus.value = {} }, 2000)
  newNames.value = []
...
export async function undoRename(historyIdx) {
  ...
  for (let i = 0; i < entry.items.length; i++) {
    ...
    try {
      const res = await apiFetch('/api/media/rename-batch', { method: 'POST', body: JSON.stringify({ items: [{ old_path: undoPath, new_name: item.oldName }], cat: entry.cat }) })
      const results = await res.json()
      if (results[0]?.success) ok++; else err++
    } catch { err++ }
  }
  endProgress()
  if (ok > 0) { renameHistory.value.splice(historyIdx, 1); _saveRenameHistory() }
...
export function clearRenameHistory() { renameHistory.value = []; _saveRenameHistory(); showToast(_t('mediaManager.historyCleared'), TOAST_TYPE.OK) }

// Drag-to-reorder (colonne right uniquement)
export function dStart(i) { dragSrc.value = i }
export function dDrop(toI) {
  if (dragSrc.value === null || dragSrc.value === toI) { dragSrc.value = null; return }
  const m = newNames.value.splice(dragSrc.value, 1)[0]
  newNames.value.splice(toI, 0, m)
  dragSrc.value = null
}
export function removeRight(i) { newNames.value.splice(i, 1) }
export function clearRight() { newNames.value = [] }
