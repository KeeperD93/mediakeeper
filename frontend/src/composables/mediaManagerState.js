...
export function setT(t) { _tImpl = t }
...
// Tooltip
export const tooltip = ref({ visible: false, title: '', year: '', vote: '', overview: '', x: 0, y: 0 })
...
} catch { renameHistory.value = [] }
export function _saveRenameHistory() { try { localStorage.setItem('mk_rename_history', JSON.stringify(renameHistory.value.slice(0, 50))) } catch { /* silent: localStorage quota/privacy mode */ } }

// ─── HISTORIQUE MOVE ───
export const moveHistory = ref([])
try { moveHistory.value = JSON.parse(localStorage.getItem('mk_move_history') || '[]').filter(e => Date.now() - e.timestamp < _HIST_TTL).slice(0, 30) } catch { moveHistory.value = [] }
export function _saveMoveHistory() { try { localStorage.setItem('mk_move_history', JSON.stringify(moveHistory.value.slice(0, 30))) } catch { /* silent: localStorage quota/privacy mode */ } }
...
function _loadAnomalyRules() {
  try { return { ...DEFAULT_ANOMALY_RULES, ...JSON.parse(localStorage.getItem('mk_anomaly_rules') || '{}') } } catch { return { ...DEFAULT_ANOMALY_RULES } }
}
...
export const DEFAULT_PROFILES = [
  { id: 'default-fr', name: 'FR Standard', builtin: true, config: { movie: '{t} ({y})', tv: '{n} - {s00e00} - {t}', lang: 'fr-FR' } },
  { id: 'default-vostfr', name: 'VOSTFR', builtin: true, config: { movie: '{t} ({y})', tv: '{n} - {s00e00} - {t} VOSTFR', lang: 'fr-FR' } },
  { id: 'default-en', name: 'EN Original', builtin: true, config: { movie: '{t} ({y})', tv: '{n} - {s00e00} - {t}', lang: 'en-US' } },
]
export const renameProfiles = ref([])
try { renameProfiles.value = JSON.parse(localStorage.getItem('mk_rename_profiles') || '[]') } catch { renameProfiles.value = [] }
export function _saveProfiles() { try { localStorage.setItem('mk_rename_profiles', JSON.stringify(renameProfiles.value)) } catch { /* silent: localStorage quota/privacy mode */ } }

// ─── CUSTOM RENAME RULES ───
export const customRules = ref([])
try { customRules.value = JSON.parse(localStorage.getItem('mk_custom_rules') || '[]') } catch { customRules.value = [] }
export function _saveCustomRules() { try { localStorage.setItem('mk_custom_rules', JSON.stringify(customRules.value)) } catch { /* silent: localStorage quota/privacy mode */ } }
...
export function setExtractionCancelled(v) { extractionCancelled = v }

// Metadata file
export const fileMetaModal = ref({ show: false, file: null, loading: false, data: null, parsed: null })

// Audio tags
export const _ALL_AUDIO_TAGS = ['MULTI', 'VOSTFR', 'VOSTA', 'VFF', 'VFI', 'VO', 'VF', 'VF2', 'DUBBED', 'SUB', 'OV', 'VOSE', 'CAST', 'DUBLADO', 'LEG', 'RAW']
...
export function setProgress(pct) { progress.value = pct }
export function endProgress() { progress.value = 100; _progressTimer = setTimeout(() => { progress.value = 0 }, 700) }
