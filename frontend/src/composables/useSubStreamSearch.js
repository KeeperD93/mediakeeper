...
export function useSubStreamSearch() {
  ...
  async function doStreamSearch() {
    ...
    try {
      ...
      if (d) {
        streamResults.value = d.items || []
        streamTotal.value = d.total || 0
        _streamOffset = streamResults.value.length
      }
    } catch {
  ...
  async function loadMoreStreams() {
    ...
    try {
      const params = new URLSearchParams({
        query: streamQuery.value.trim(),
        stream_type: streamTypeFilter.value,
        start: String(_streamOffset),
        limit: '50',
      })
      const d = await apiGet(`/api/subtitles/search-streams?${params}`)
      if (d && d.items) {
        streamResults.value.push(...d.items)
        _streamOffset += d.items.length
      }
    } catch {
      /* silent: stream search fetch */
    }
    streamSearching.value = false
  }
  ...
  function toggleStreamSelect(itemId, index, item, stream) {
    const idx = selectedStreams.value.findIndex(
      s => s.item_id === itemId && s.stream_index === index,
    )
    if (idx >= 0) selectedStreams.value.splice(idx, 1)
    else
      selectedStreams.value.push({ item_id: itemId, stream_index: index, name: item.name, stream })
  }

  const allSelected = computed(
    () =>
      streamResults.value.length > 0 &&
      streamResults.value.every(item =>
        item.matching_streams.every(s => isStreamSelected(item.item_id, s.index)),
      ),
  )
  ...
  async function removeSingleStream(itemId, index, stream) {
    ...
    try {
      const d = await apiPost('/api/subtitles/remove-stream', {
        item_id: itemId,
        stream_index: index,
      })
      if (d && d.success) {
        ...
        if (item) {
          ...
          if (!item.matching_streams.length)
            streamResults.value = streamResults.value.filter(i => i.item_id !== itemId)
        }
  ...
  async function batchRemove() {
    ...
    try {
      const d = await apiPost('/api/subtitles/batch-remove-streams', {
        operations: selectedStreams.value.map(s => ({
          item_id: s.item_id,
      ...
      if (d) {
        ...
        showToast(
          `${ok} ${t('subtitles.streamRemoved').toLowerCase()}${fail ? `, ${fail} ${t('common.error').toLowerCase()}` : ''}`,
          ok ? TOAST_TYPE.OK : TOAST_TYPE.ERR,
        )
        const removed = new Set((d.success || []).map(s => `${s.item_id}:${s.stream_index}`))
        for (const item of streamResults.value) {
          item.matching_streams = item.matching_streams.filter(
            s => !removed.has(`${item.item_id}:${s.index}`),
          )
        }
        streamResults.value = streamResults.value.filter(i => i.matching_streams.length > 0)
        selectedStreams.value = selectedStreams.value.filter(
          s => !removed.has(`${s.item_id}:${s.stream_index}`),
        )
      }
    } catch {
      showToast(t('common.error'), TOAST_TYPE.ERR)
