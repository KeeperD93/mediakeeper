import { ref } from 'vue'
import {
  Calendar, CalendarClock, CircleCheck, CirclePlay, ClipboardCheck, Clock, Copy,
  Database, Grid3x3, ShieldCheck, TrendingUp, Users, Zap,
...
export const WIDGET_REGISTRY = {
  ...
  statDuration: { label: 'Total duration',     defaultLayout: { x: 25, y: 0,  w: 4,  h: 5 },  minW: 3,  minH: 4 },
  statStorage:  { label: 'Media storage',      defaultLayout: { x: 29, y: 0,  w: 4,  h: 5 },  minW: 3,  minH: 4 },
  statDuplicates: { label: 'Duplicates',         defaultLayout: { x: 33, y: 0,  w: 3,  h: 5 },  minW: 3,  minH: 4 },
  topUsers:     { label: 'Top users',          defaultLayout: { x: 21, y: 5,  w: 8,  h: 10 }, minW: 6,  minH: 9,  maxH: 12 },
  heatmap:      { label: 'Activity heatmap',   defaultLayout: { x: 29, y: 5,  w: 7,  h: 10 }, minW: 6,  minH: 6 },
  upcoming:     { label: 'Upcoming releases',  defaultLayout: { x: 11, y: 30, w: 25, h: 15 }, minW: 6,  minH: 6 },
  portalAction:     { label: 'Requests — action',     defaultLayout: { x: 11, y: 0,  w: 10, h: 7 },  minW: 10, minH: 7 },
  portalEngagement: { label: 'Requests — activity',   defaultLayout: { x: 11, y: 7,  w: 10, h: 8 },  minW: 10, minH: 7 },
  portalEvents:     { label: 'Portal — upcoming events', defaultLayout: { x: 28, y: 15, w: 8,  h: 15 }, minW: 4,  minH: 7 },
  linkWatchlist:      { label: 'Tracking',              defaultLayout: { x: 11, y: 15, w: 7,  h: 4 },  minW: 6,  minH: 4 },
  healthScore:        { label: 'Media health',          defaultLayout: { x: 18, y: 15, w: 7,  h: 4 },  minW: 6,  minH: 4 },
}
...
export function useDashboardLayout() {
  ...
  async function loadLayout() {
    try {
      ...
      if (data && data.positions && Object.keys(data.positions).length > 0 && data.v === LAYOUT_VERSION) {
  ...
  async function saveLayoutNow() {
    ...
    try {
      await apiPost('/api/settings/dashboard', { hidden: hidden.value, positions, v: LAYOUT_VERSION })
    } catch (e) {
  ...
  function toggleWidget(id) {
    ...
    if (idx >= 0) {
      ...
      if (def) {
        layout.value.push({
          i: id, x: def.defaultLayout.x, y: def.defaultLayout.y,
  ...
  return {
    editing, hidden, layout, loaded,
    loadLayout, saveLayout, toggleWidget, resetLayout, onLayoutUpdated,
    WIDGET_REGISTRY, WIDGET_ICONS,
  }
}
