  ...
  async function listRecursive(sp) {
    try {
      const data = await apiGet(
        `/api/media/files/${activeCat.value}?subpath=${encodeURIComponent(sp)}`,
      )
      ...
      console.error('[mediaManagerMatch.listRecursive] failed to expand folder', e)
    }
  }
...
export function cancelExtraction() {
  setExtractionCancelled(true)
}

export async function extractFromSubfolders() {
  const dirs = checkedDirs.value
  if (!dirs.length) {
    showToast(_t('mediaManager.selectFoldersToExtract'), TOAST_TYPE.ERR)
    return
  }
  ...
  try {
    ...
    if (!rootData?.path) {
      showToast(_t('mediaManager.rootPathError'), TOAST_TYPE.ERR)
      extractionRunning.value = false
      return
    }
    destPath = subPath.value
      ? rootData.path.replace(/\/$/, '') + '/' + subPath.value.replace(/^\//, '')
      : rootData.path
  } catch {
  ...
  async function listRecursive(sp) {
    ...
    try {
      ...
      /* silent: skip unreadable subfolder, continue traversal */
    }
  }
  setProgress(5)
  for (const dir of dirs) {
  ...
  for (let i = 0; i < filesToMove.length; i++) {
    ...
    try {
      const res = await apiFetch('/api/media/move', {
        method: 'POST',
        body: JSON.stringify({ src_path: filesToMove[i].path, dest_folder: destPath }),
      })
      ...
      if (data.error) errCount++
        ...
        ok++
        movedItems.push({
          name: filesToMove[i].name,
          oldPath: filesToMove[i].path,
          newPath: destPath + '/' + filesToMove[i].name,
        })
      }
    } catch {
      errCount++
  ...
  extractionRunning.value = false
  if (mmState.extractionCancelled)
    showToast(_t('mediaManager.extractionCancelledPartial', { count: ok }), TOAST_TYPE.WARN)
  else if (errCount === 0)
    showToast(_t('mediaManager.filesExtracted', { count: ok }), TOAST_TYPE.OK)
  else
    showToast(
      _t('mediaManager.partialResult', { ok, errors: errCount }),
      errCount > 0 && ok === 0 ? TOAST_TYPE.ERR : TOAST_TYPE.WARN,
    )
  checked.value = new Set()
  loadFiles()
}
...
export async function doMatch(addMode = false) {
  ...
  if (addMode) {
    ...
    if (checkedDirs.value.length > 0) {
      ...
      if (!matchFiles?.length) {
        showToast(_t('mediaManager.noFilesInSelectedFolders'), TOAST_TYPE.ERR)
        return
      }
    } else if (checkedFiles.value.length > 0) {
      matchFiles = checkedFiles.value
    } else {
      filtered.value.forEach((f, i) => {
        if (f.type === FILE_TYPE.FILE) checked.value.add(i)
      })
      matchFiles = [...checked.value]
        .sort((a, b) => a - b)
        .map(i => filtered.value[i])
        .filter(f => f?.type === FILE_TYPE.FILE)
    }
    if (!matchFiles?.length) {
      showToast(_t('mediaManager.noVideoFilesFound'), TOAST_TYPE.ERR)
      return
    }
    newNames.value = []
    await _doMatchFromFiles(item, fr, year, matchFiles)
    const tagsToSave = {}
    newNames.value.forEach(n => {
      if (n.category) {
        tagsToSave[n.name] = n.category
        if (n.oldName) tagsToSave[n.oldName] = n.category
      }
    })
  ...
  newNames.value = newNames.value.filter(n => {
    if (n.path) {
      if (seenPaths.has(n.path)) return false
...
async function _doMatchFromFiles(item, fr, year, matchFiles) {
    ...
    for (const f of matchFiles) {
      ...
      if (matchFiles.length > 1) {
        const guessTitle = f.name
          ...
          .replace(
            /\b(FRENCH|MULTi|VOSTFR|BluRay|WEBRip|HDTV|x264|x265|H264|H265|1080p|720p|4K|HDR|PROPER|REPACK|EXTENDED|Complete|Integrale|Complete|Coffret|Collection|Pack)\b.*/i,
            '',
          )
          .trim()
        try {
          ...
          const results = await apiGet(
            `/api/media/tmdb/search/movie?q=${encodeURIComponent(guessTitle)}&language=${encodeURIComponent(lang)}`,
          )
          if (results?.length) {
            title = results[0].title
            movieYear = results[0].year || ''
      ...
      const res = await apiFetch('/api/media/build/movie', {
        method: 'POST',
        body: JSON.stringify({ title, year: movieYear, ext }),
      })
  ...
  for (const sNum of neededSeasons) {
    try {
      const data = await apiGet(
        `/api/media/tmdb/tv/${item.id}/season/${sNum}?language=${encodeURIComponent(_lang)}`,
      )
  ...
  matchFiles.forEach(f => {
    ...
    if (mSE) {
      fileSeason = parseInt(mSE[1])
      fileEpNum = parseInt(mSE[2])
    } else if (mXx) {
      fileSeason = parseInt(mXx[1])
      fileEpNum = parseInt(mXx[2])
