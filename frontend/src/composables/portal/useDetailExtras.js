...
export function useDetailExtras() {
  ...
  async function load(media) {
    ...
    const [dir, act, col] = await Promise.all([
      directorId
        ? apiGet(`/api/portal/catalog/person/${directorId}?role=director`).catch(() => null)
        : null,
      actorId
        ? apiGet(`/api/portal/catalog/person/${actorId}?role=acting`).catch(() => null)
        : null,
      collectionId
        ? apiGet(`/api/portal/catalog/collection/${collectionId}`).catch(() => null)
        : null,
    ])
    ...
    if (col)
      collection.value = { ...col, items: (col.items || []).filter(i => !isSameAsCurrent(i)) }
  }
