...
import {
  apiGet, _t,
  subPath, checked, selectedTmdb, tmdbResults, currentSeason, seasons, showSeasonPanel,
  searchType, tooltip, newNames, anomalyRules,
  _autoSearchState,
...
export const canGenerate = computed(() => selectedTmdb.value && (checked.value.size > 0 || searchType.value === 'tv'))
...
export async function doSearch(autoSelect = false, query = '') {
  ...
  tmdbResults.value = []; selectedTmdb.value = null; hideTooltip()
  try {
    const ep = searchType.value === 'movie' ? 'movie' : 'tv'
    const lang = anomalyRules.value._tmdbLang || 'fr-FR'
    const data = await apiGet(`/api/media/tmdb/search/${ep}?q=${encodeURIComponent(query)}&language=${encodeURIComponent(lang)}`)
    if (!data?.length) return
    tmdbResults.value = data.slice(0, 10)
    if (autoSelect) pickTMDB(0)
  } catch { /* silent: TMDB search miss → empty results panel */ }
}
...
export function pickTMDB(idx) {
  ...
  else { showSeasonPanel.value = false; currentSeason.value = null }
}

export async function loadSeasons(tmdbId) {
  showSeasonPanel.value = true; currentSeason.value = 1; seasons.value = []
  const lang = anomalyRules.value._tmdbLang || 'fr-FR'
  try {
    const data = await apiGet(`/api/media/tmdb/tv/${tmdbId}/seasons?language=${encodeURIComponent(lang)}`)
  ...
  } catch { /* silent: TMDB seasons unavailable → panel stays empty */ }
}

export function showTooltipTmdb(item, e) {
  const ov = (item.overview || '').slice(0, 240) + ((item.overview || '').length > 240 ? '…' : '')
  tooltip.value = {
    visible: true, title: item.title || '', year: item.year || '—',
    vote: item.vote ? item.vote.toFixed(1) : '—', overview: ov,
...
export function moveTooltip(e) {
  ...
  tooltip.value.x = x; tooltip.value.y = y
}
export function hideTooltip() { tooltip.value.visible = false }
...
export function autoSearch(tmdbQueryEl) {
  ...
  _autoSearchState.timer = setTimeout(async () => {
    ...
    const cFiles = checkedFiles.value, cDirs = checkedDirs.value
    let detected = ''
    ...
    if (cDirs.length > 0) detected = pickConsensus(cDirs)
      ...
      for (let i = segments.length - 1; i >= 0; i--) {
        ...
        if (candidate) { detected = candidate; break }
      }
    }
...
export async function fuzzyTmdbSearch(query, type = 'movie') {
  ...
  try {
    ...
    const fallback = await apiGet(`/api/media/tmdb/search/${ep}?q=${encodeURIComponent(words[0])}&language=${encodeURIComponent(lang)}`)
