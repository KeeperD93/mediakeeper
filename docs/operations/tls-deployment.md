...
violation.**
The shipped policy allows: `'self'` for scripts, `'self'` plus Google
Fonts for styles and `'self'` plus TMDB/Imgur for images. If you
deploy a customisation that loads other origins, switch to
`MK_CSP_MODE=report-only` for a transition window, observe the
