...
async def get_trending(db: AsyncSession, page: int = 1) -> list[dict]:
    return await _fetch_list(db, "/trending/all/week", page)


@cached_tmdb_list(_LIST_TTL_SEC)
async def get_popular_movies(db: AsyncSession, page: int = 1) -> list[dict]:
    return await _fetch_list(db, "/movie/popular", page)


@cached_tmdb_list(_LIST_TTL_SEC)
async def get_popular_tv(db: AsyncSession, page: int = 1) -> list[dict]:
    return await _fetch_list(db, "/tv/popular", page)


@cached_tmdb_list(_LIST_TTL_SEC)
async def get_top_rated(db: AsyncSession, page: int = 1) -> list[dict]:
    return await _fetch_list(db, "/movie/top_rated", page)


@cached_tmdb_list(_LIST_TTL_SEC)
async def get_top_rated_year(db: AsyncSession, page: int = 1) -> list[dict]:
...
async def get_oscar_winners(db: AsyncSession, page: int = 1) -> list[dict]:
    """
...
@cached_tmdb_list(_LIST_TTL_SEC)
async def get_family(db: AsyncSession, page: int = 1) -> list[dict]:
...
@cached_tmdb_list(_LIST_TTL_SEC)
async def get_upcoming(db: AsyncSession, page: int = 1) -> list[dict]:
...
async def get_by_provider(
    db: AsyncSession, provider_id: int, media_type: str = "movie", page: int = 1,
) -> list[dict]:
...
async def get_media_videos(db: AsyncSession, media_type: str, tmdb_id: int) -> list[dict]:
...
async def _fetch_list(db: AsyncSession, endpoint: str, page: int) -> list[dict]:
    return await _fetch_list_params(db, endpoint, page, {})
