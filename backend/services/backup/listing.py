...
from ._state import get_current_backup_dir
...
def list_backups() -> list[dict]:
    ...
    try:
        ...
        logger.warning(f"[backup] Directory unreachable for backup listing: {backup_dir} ({e})")
        return []
    backups = []
...
def get_backup_path(filename: str) -> Optional[Path]:
    """Return le path full d'un backup si le file existe."""
    if not _BACKUP_FILENAME_RE.fullmatch(filename or ""):
        return None
    backup_dir = get_current_backup_dir().resolve(strict=False)
...
def apply_retention(retention_days: int = 30) -> int:
    ...
    for f in backup_dir.glob("mediakeeper_backup_*.zip"):
        ...
        if mtime < cutoff:
            f.unlink()
            removed += 1
            logger.info(f"[backup] Retention: removed {f.name}")
    return removed


def apply_retention_by_count(max_count: int = 10) -> int:
    ...
    for f in files[max_count:]:
        f.unlink()
        removed += 1
        logger.info(f"[backup] Count retention: removed {f.name}")
    return removed
